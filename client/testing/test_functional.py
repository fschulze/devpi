...
def test_switching_user_preserves_auth(existing_user_id, out_devpi, url_of_liveserver):
    import re
    result = out_devpi("use", url_of_liveserver)
    (user1,) = re.search(
        r'\(logged in as (.+?)\)', result.stdout.str()).groups()
    assert user1 != existing_user_id
    result = out_devpi("use", "-u", existing_user_id, url_of_liveserver)
    result.stdout.fnmatch_lines("*(not logged in)")
    result = out_devpi("login", existing_user_id, "--password", "1234")
    (user2,) = re.search(
        r"logged in '(.+?)'", result.stdout.str()).groups()
    assert user1 != user2
    assert user2 == existing_user_id
    result = out_devpi("use", "-u", user1, url_of_liveserver)
