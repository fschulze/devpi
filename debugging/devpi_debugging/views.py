        ...
        with storage.get_connection() as conn:
            if conn._sqlaconn.dialect.name == "sqlite":
                q = (
                    ...
                    .where(sa.func.hex(storage.ulid_changelog_table.c.value).contains(sa.func.hex(query.encode())))
                    .order_by(storage.ulid_changelog_table.c.serial)
                    .distinct())
            elif conn._sqlaconn.dialect.name == "postgresql":
                q = (
                    sa.select(storage.ulid_changelog_table.c.serial)
                    .where(sa.func.position(sa.literal(query.encode()).op("IN")(storage.ulid_changelog_table.c.value)) > 0)
                    .order_by(storage.ulid_changelog_table.c.serial)
                    .distinct())
            else:
                q = (
                    sa.select(storage.ulid_changelog_table.c.serial)
                    .where(storage.ulid_changelog_table.c.value.contains(query.encode()))
                    .order_by(storage.ulid_changelog_table.c.serial)
                    .distinct())
        ...
        for keydata in conn.iter_changes_at(serial):
            ...
            if keydata.back_serial >= 0:
                prev_formatted = pformat(
                    None
                    if (v := conn.get_key_at_serial(key, keydata.back_serial).mutable_value) is deleted else
                    v)
            ...
            changes.append(dict(
                fragment=sha256(f"{keydata.keyname}-{keydata.relpath}".encode()).hexdigest(),
