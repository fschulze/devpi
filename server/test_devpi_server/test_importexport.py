...
class TestImportExport:
        ...
        class ImpExp:
            ...
            def import_testdata(self, name, options=()):
                ...
                with importlib.resources.as_file(
                        files / 'importexportdata') as path:
                    serverdir = gentmp()
                    argv = [
                        "devpi-import",
                        "--no-events",
    ...
    def test_mirrordata(self, impexp):
        ...
        with mapp.xom.keyfs.read_transaction():
            ...
            assert link.entry.hashes[hash_type] == hash_value
    ...
    def test_upload_releasefile_with_toxresult(self, impexp, tox_result_data):
        ...
        path, = mapp1.get_release_paths("hello")
        path = path.strip("/")
        toxresult_dump = json.dumps(tox_result_data).encode()
        hashes = get_hashes(toxresult_dump)
        ...
        assert toxresult_link["hashes"] == hashes
        sleep(1.5)
        impexp.export()
        mapp2 = impexp.new_import()
        with mapp2.xom.keyfs.read_transaction():
    ...
    def test_export_hard_links(self, makeimpexp):
        ...
        assert impexp.exportdir.join(
          'user1', 'dev', 'he-llo-1.0.doc.zip').stat().nlink == 2
        assert impexp.exportdir.join(
    ...
    def test_import_hard_links(self, makeimpexp):
        ...
        assert impexp.exportdir.join(
            'dataindex.json').stat().nlink == 1
        assert impexp.exportdir.join(
            'user1', 'dev', 'he-llo-1.0.doc.zip').stat().nlink == 1
        assert impexp.exportdir.join(
