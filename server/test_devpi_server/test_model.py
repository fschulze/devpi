...
class TestStage:
    ...
    def test_project_versiondata_shadowed(self, pypistage, stage):
        ...
        pypistage.mock_simple(
            "someproject", "<a href='someproject-1.0.zip' /a>")
        content = b"123"
    ...
    def test_filename_version_mangling_issue68(self, stage):
        ...
        content = b""
        stage.store_releasefile(
            "hello", "1.0-test", "hello-1.0_test.whl", content, hashes=get_hashes(content))
    ...
    def test_get_last_change_serial_perstage(self, xom):
        ...
        actions = [
            ...
            ('store_releasefile', ("pkg", "1.0", "pkg-1.0.zip", content), dict(hashes=hashes)),
            ('store_doczip', ("pkg", "1.0", content), dict(hashes=hashes)),
            ('set_versiondata', (dict(name="hello", version="1.0"),), {}),
            ('store_releasefile', ("hello", "1.0", "hello-1.0.zip", content), dict(hashes=hashes)),
            ('store_releasefile', ("pkg", "1.0", "pkg-1.0.tar.gz", content), dict(hashes=hashes))]
    ...
    def test_get_last_project_change_serial_perstage(self, xom):
        ...
        with xom.keyfs.read_transaction() as tx:
            ...
            assert stage.get_last_project_change_serial_perstage('pkg') == (first_serial + 2)
        with xom.keyfs.write_transaction() as tx:
            # no change in db yet
            assert tx.at_serial == (first_serial + 2)
        ...
        with xom.keyfs.write_transaction() as tx:
            ...
            assert tx.at_serial == (first_serial + 6)
            content = b'123'
            link = stage.store_releasefile('pkg', '1.0', 'pkg-1.0.zip', content, hashes=get_hashes(content))
        with xom.keyfs.read_transaction() as tx:
        ...
        with xom.keyfs.write_transaction() as tx:
            ...
            assert tx.at_serial == (first_serial + 7)
            content = b'123'
            other_link = stage.store_releasefile('other', '1.1', 'other-1.1.zip', content, hashes=get_hashes(content))
        with xom.keyfs.read_transaction() as tx:
...
class TestLinkStore:
    ...
    def test_store_file(self, linkstore):
        ...
        linkstore.create_linked_entry(
            ...
            content_or_file=content, hashes=get_hashes(content))
        linkstore.create_linked_entry(
    ...
    def test_toxresult_create_remove(self, linkstore):
        ...
        linkstore.create_linked_entry(
            rel="releasefile", basename="proj1-1.0.zip",
            content_or_file=content1, hashes=get_hashes(content1))
        content2 = b'456'
        linkstore.create_linked_entry(
            rel="releasefile", basename="proj1-1.1.zip",
            content_or_file=content2, hashes=get_hashes(content2))
