...
class TestIndex:
    ...
    def test_normalized_project_name(self, xom: XOM) -> None:
        ...
        with keyfs.write_transaction():
            ...
            key_project = keyfs.schema.PROJECT.search(
                user=stage.username, index=stage.index
            )
        with keyfs.read_transaction():
            name = normalize_name("Mr.Foo")
            ((k, v),) = key_project.with_resolved_parent().iter_ulidkey_values()
    ...
    def test_inheritance_cycle(self, model, pypistage):
        from devpi_server.model.inheritance import InheritanceCycle
    ...
    def test_long_description(self, stage):
        ...
        metadata = (
            stage.key_versionmetadata("foo", "1.0").with_resolved_parent().get_mutable()
        )
    ...
    def test_project_trust_inheritance_none(self, pypistage, stage, user):
        user.create_stage(index="dev2", bases=("root/pypi",))
        stage_dev2 = user.getstage("dev2")
        stage.modify(
            trust_inheritance_rules_from="type:not remote", bases=(stage_dev2.name,)
        )
        pypistage.mock_simple("someproject", '<a href="someproject-1.1.zip" />')
        register_and_store(stage, "someproject-1.0.zip", b"123")
        links = stage.get_releaselinks("someproject")
    ...
    def test_project_trust_inheritance_all(self, pypistage, stage, user):
        user.create_stage(index="dev2", bases=("root/pypi",))
        stage_dev2 = user.getstage("dev2")
        stage.modify(
            trust_inheritance_rules_from="type:not remote", bases=(stage_dev2.name,)
    ...
    def test_notrust_inheritance_two_remotes(self, http, stage, user):
    ...
    def test_trust_inheritance(self, pypistage, stage, user):
        ...
        stage2 = user.create_stage(
            index="inheriting",
            bases=(stage.name,),
            trust_inheritance_rules_from="type:not remote",
        )
        assert "trust_inheritance_rules_from" not in stage.ixconfig
    ...
    def test_trust_inheritance_two_remotes(self, http, stage, user):
        remote1 = user.create_stage(
            "remote1", **udict(remote_url="http://pypi.org/simple", type="remote")
        )
        remote2 = user.create_stage(
    ...
    def test_getdoczip(self, stage, tmpdir):
        ...
        stage.store_doczip(
            "pkg1", "1.0", content, hashes=get_hashes(content), size=get_size(content)
        )
    ...
    def test_storetoxresult(self, stage):
        ...
        tlink = stage.store_toxresult(
            link, content, hashes=get_hashes(content), size=get_size(content)
        )
    ...
    def test_doczip_remove_hook(self, stage, queue):
        ...
        stage.store_doczip(
            "pkg2", "1.0", content, hashes=get_hashes(content), size=get_size(content)
        )
        stage.xom.keyfs.commit_transaction_in_thread()
        nstage, name, version, link = queue.get()
        assert name == "pkg2"
    ...
    def test_get_last_change_serial_perstage(self, xom):
        ...
        actions: list = [
            ...
            (
                ...
                dict(hashes=hashes, size=size),
            ),
            ("store_doczip", ("pkg", "1.0", content), dict(hashes=hashes, size=size)),
        ...
        with xom.keyfs.write_transaction():
            ...
            stage.store_toxresult(link, b"{}", hashes=get_hashes(b"{}"), size=2)
        assert current_serial == xom.keyfs.get_current_serial() - 1
        current_serial = xom.keyfs.get_current_serial()
        with xom.keyfs.read_transaction():
            assert stage.get_last_change_serial_perstage() == current_serial
    ...
    def test_get_last_project_change_serial_perstage(self, xom):
        ...
        with xom.keyfs.write_transaction() as tx:
            # no change in db yet
            assert tx.at_serial == (first_serial + 1)
            stage.key_project("pkg").with_resolved_parent().delete()
            assert stage.list_projects_perstage() == {}
        with xom.keyfs.read_transaction() as tx:
