...
def pytest_configure(config):
    config.addinivalue_line(
        "markers",
        "mock_frt_http: mock FileReplicationThread.http")
    config.addinivalue_line(
        "markers",
...
def makexom(request, gen_path, http, monkeypatch, storage_args, storage_plugin):
    def makexom(opts=(), http=http, plugins=()):  # noqa: PLR0912
        ...
        if request.node.get_closest_marker("storage_with_filesystem") and not config.storage_info.storage_with_filesystem:
...
def httpget(http):
    warnings.warn(
        "The httpget fixture is deprecated, "
        "use http instead",
        DeprecationWarning,
        stacklevel=2)
    return http
...
def add_pypistage_mocks(monkeypatch, http):
    ...
    def mock_simple(self, name, text=None, pypiserial=10000, **kw):
        ...
        return self.xom.http.mock_simple(
