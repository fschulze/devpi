...
class TestChangelog:
    ...
    def test_accept_header(self, auth_serializer, testapp):
        ...
        r = testapp.xget(
            403, "/+changelog/0", headers={"Accept": REPLICA_CONTENT_TYPE_V2}
        )
        token = auth_serializer.dumps(self.replica_uuid)
        r = testapp.xget(
            200,
...
class TestReplicaThread:
    ...
    def test_thread_run_recovers_from_error(
        self, mock, rt, mockchangelog, monkeypatch, caplog, xom
    ):
        ...
        msgs = [
            x.msg
            for x in caplog.getrecords(r"\[REP\].*http://localhost/\+changelog/0-")
        ]
        assert msgs == [
            "[REP] fetching %s",
...
class TestProxyViewToPrimary:
    def test_write_proxies(self, makexom, blank_request, monkeypatch):
        ...
        xom.http.mockresponse(
            "http://localhost/blankpath", content=b"", headers={"X-DEVPI-SERIAL": "10"}
        )
        l = []
    ...
    def test_write_proxies_redirect(self, makexom, blank_request, monkeypatch):
        ...
        xom.http.mockresponse(
            ...
            code=302,
            content=b"",
            headers={"X-DEVPI-SERIAL": "10", "location": "http://localhost/hello"},
        )
        l = []
        monkeypatch.setattr(xom.keyfs, "wait_tx_serial",
                            lambda x: l.append(x))
...
@pytest.fixture
def make_replica_xom(http, makexom, monkeypatch, secretfile):
    def make_replica_xom(options=(), *, mock_frt_http=False):
        ...
        if mock_frt_http:
            monkeypatch.setattr(replica_xom.frt, "http", http)
...
class TestSkipIndexes:
    def test_skip_indexes_option(self, make_replica_xom):
        ...
        replica_xom = make_replica_xom(
            options=["--file-replication-skip-indexes", "remote, root/pypi"]
        )
...
class TestUseExistingFiles:
    ...
    def test_use_existing_files_bad_data(
        ...
        replica_xom.frt.http.mockresponse(f"http://localhost{path}", content=content1)
        # now sync the replica, if the file is not found there will be an error
        # because httpget is mocked
        caplog.clear()
        replay(xom, replica_xom, events=False)
    ...
    @pytest.mark.skipif(not hasattr(os, 'link'),
                        reason="OS doesn't support hard links")
    def test_hardlink_bad_data(self, caplog, make_replica_xom, mapp, tmpdir, xom):
        # this will be the folder to find existing files in the replica
        existing_base = tmpdir.join('existing').ensure_dir()
        # prepare data on primary
        mapp.create_and_use()
        ...
        replica_xom = make_replica_xom(
            options=[
                "--replica-file-search-path",
                existing_base.strpath,
                "--hard-links",
            ],
            mock_frt_http=True,
        )
...
class TestFileReplication:
    ...
    def test_fetch_later_deleted(self, caplog, gen, xom, replica_xom):
        ...
        replica_xom.frt.http.mockresponse(primary_file_path, status_code=410)
    ...
    def test_fetch_pypi_nomd5(self, gen, xom, replica_xom):
        ...
        with pytest.raises(
            MissingFileException, match=r"missing file .* at serial 2"
        ) as e:
    ...
    def test_cache_remote_file_fetch_original(self, xom, replica_xom, gen,
        ...
        with replica_xom.keyfs.read_transaction():
            ...
            url = replica_xom.config.primary_url.joinpath(entry.relpath).url
            pypistage.xom.http.mockresponse(url, status_code=500)
            pypistage.xom.http.mockresponse(entry.url, headers=headers, content=content)
            stage = replica_xom.model.getstage('root/pypi')
...
class TestFileReplicationSharedData:
    ...
    def test_remote_priority(self, shared_data):
        from itertools import permutations
        result = []

        remote_file = "root/pypi/+f/3f8/3058ac9076112/pytest-2.0.0.zip"
        ...
        shared_data.set_index_type_for("root/pypi", "remote")
        ...
        cases: list = []
        cases.extend(
            permutations(((remote_file, 0), (local_file, 0), (deleted_file, 0)))
        )
        cases.extend(
            permutations(((remote_file, 1), (local_file, 0), (deleted_file, 0)))
        )
        cases.extend(
            permutations(((remote_file, 0), (local_file, 1), (deleted_file, 0)))
        )
        cases.extend(
            permutations(((remote_file, 0), (local_file, 0), (deleted_file, 1)))
        )
        cases.extend(
            permutations(((remote_file, 0), (local_file, 1), (deleted_file, 2)))
        )
        cases.extend(
            permutations(((remote_file, 0), (local_file, 2), (deleted_file, 1)))
        )
        cases.extend(
            permutations(((remote_file, 1), (local_file, 0), (deleted_file, 2)))
        )
        cases.extend(
            permutations(((remote_file, 1), (local_file, 2), (deleted_file, 0)))
        )
        cases.extend(
            permutations(((remote_file, 2), (local_file, 0), (deleted_file, 1)))
        )
        cases.extend(
            permutations(((remote_file, 2), (local_file, 1), (deleted_file, 0)))
        )
        for (relpath1, serial1), (relpath2, serial2), (relpath3, serial3) in cases:
            ...
            assert result == [local_file, remote_file, deleted_file]
            result.clear()
    ...
    def test_recreated_index(self, mapp, monkeypatch, shared_data, xom):
        from devpi_server.replica import IndexType
        ...
        mapp.create_index("dev", indexconfig=dict(type="remote"))
        ...
        with xom.keyfs._storage.get_connection() as conn:
            for serial in range(xom.keyfs.get_next_serial()):
                ...
                for keydata in conn.iter_changes_at(serial):
                    changes[serial][keydata.key] = (keydata.value, keydata.back_serial)
        ...
        serial += 1
        assert shared_data.index_types.get("root/pypi") == IndexType("remote")
        assert shared_data.index_types.get("user/dev") is None
        # user creation
        shared_data.on_import(serial, changes[serial])
        serial += 1
        ...
        serial += 1
        assert shared_data.index_types.get("root/pypi") == IndexType("remote")
        assert shared_data.index_types.get("user/dev") == IndexType("local")
        ...
        serial += 1
        assert shared_data.index_types.get("root/pypi") == IndexType("remote")
        assert shared_data.index_types.get("user/dev") == IndexType("remote")
        # user deletion
        shared_data.on_import(serial, changes[serial])
        serial += 1
        assert shared_data.index_types.get("root/pypi") == IndexType("remote")
