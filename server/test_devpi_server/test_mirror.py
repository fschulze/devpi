...
class TestExtPYPIDB:
    def test_parse_pep691(self, pypistage):
        ...
        assert link.best_available_hash_spec == 'sha256=b89846ad42cfee0e44934ef77f28ad44e90b7e744041ace91047dd4c7892cc5e'
        assert link.yanked is None
    ...
    def test_parse_project_nomd5(self, pypistage):
        ...
        assert not link.best_available_hash_spec
    ...
    def test_get_versiondata(self, pypistage):
        ...
        assert not elink.hashes.best_available_spec
...
def test_redownload_locally_removed_release(file_digest, mapp, simpypi):
    ...
    file_path_info = FilePathInfo(f"+files{URL(result[0]).path}", file_digest(content))
    assert len(result) == 1
