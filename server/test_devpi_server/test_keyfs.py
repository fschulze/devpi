...
def keyfs(gen_path, pool, storage_info, storage_io_file_factory):
    keyfs = KeyFS(gen_path(), storage_info, io_file_factory=storage_io_file_factory, schema=KeyFSSchema)
...
class TestKeyFS:
    ...
    def test_keyfs_readonly(self, storage_info, storage_io_file_factory, tmpdir):
        keyfs = KeyFS(
            tmpdir, storage_info, io_file_factory=storage_io_file_factory, readonly=True, schema=KeyFSSchema
        )
...
class TestSubscriber:
    ...
    def test_last_key_serial(self, keyfs):
        key1 = keyfs.schema.named_key_factory("SOME1", "{some}/1", None, set, SetViewReadonly)
...
def test_crash_recovery(caplog, keyfs, storage_info):
    ...
    key = keyfs.schema.named_key_factory("PYPIFILE_NOMD5", "+e/{path}", None, dict, DictViewReadonly)
...
def test_keyfs_sqlite(file_digest, gen_path, sorted_serverdir):
    ...
    keyfs = KeyFS(
        ...
        io_file_factory=io_file_factory, schema=KeyFSSchema
    )
    content = b"bar"
