...
class TestKeyFS:
    ...
    def test_keyfs_readonly(self, storage_info, storage_io_file_factory, tmpdir):
        keyfs = KeyFS(tmpdir, storage_info, io_file_factory=storage_io_file_factory, readonly=True)
...
class TestTransactionIsolation:
    ...
    def test_import_changes_subscriber_error(self, keyfs, storage_info, storage_io_file_factory, tmpdir):
        pkey = keyfs.register_named_key("NAME", "hello/{name}", None, dict)
        D = pkey(name="world")
        with keyfs.write_transaction():
            D.set({1: 1})
        keyfs_serial = keyfs.get_current_serial()
        new_keyfs = KeyFS(tmpdir.join("newkeyfs"), storage_info, io_file_factory=storage_io_file_factory)
        pkey = new_keyfs.register_named_key("NAME", "hello/{name}", None, dict)
    ...
    def test_cache_interference(self, storage_info, storage_io_file_factory, tmpdir):
        ...
        keyfs1 = KeyFS(tmpdir.join("keyfs1"), storage_info, io_file_factory=storage_io_file_factory)
...
class TestSubscriber:
    ...
    def test_notifications_retried_after_exception(self, tmpdir, queue, monkeypatch, storage_info, storage_io_file_factory):
        ...
        with make_keyfs() as key1, monkeypatch.context() as m:
            m.setattr(
                key1.keyfs._storage, "_notify_on_commit", lambda _x: 0 / 0)
...
def test_crash_recovery(file_digest, keyfs, storage_info):
    ...
    with keyfs.read_transaction() as tx:
        ...
        (tmpname,) = tx.conn.iter_rel_renames(tx.at_serial)
...
def test_keyfs_sqla_lite_files(gen_path, file_digest, sorted_serverdir):
...
def test_keyfs_sqla_lite(gen_path, file_digest, sorted_serverdir):
...
def test_iter_keys_at_serial(keyfs):
    ...
    key = pkey(name="hello")
    with keyfs.read_transaction() as tx:
        assert list(tx.conn.iter_keys_at_serial([key], tx.at_serial, fill_cache=True, with_deleted=True)) == []
    with keyfs.write_transaction():
        key.set(1)
    with keyfs.read_transaction() as tx:
        (keydata,) = list(tx.conn.iter_keys_at_serial([key], tx.at_serial, fill_cache=True, with_deleted=True))
...
def test_iter_ulidkeys_at_serial(keyfs):
    ...
    key = pkey(name="hello")
    with keyfs.read_transaction() as tx:
        assert list(tx.conn.iter_ulidkeys_at_serial([key], tx.at_serial, fill_cache=True, with_deleted=True)) == []
    with keyfs.write_transaction():
        key.set(1)
    with keyfs.read_transaction() as tx:
        (key,) = list(tx.conn.iter_ulidkeys_at_serial([key], tx.at_serial, fill_cache=True, with_deleted=True))
