...
def keyfs(gen_path, pool, storage, storage_io_file_factory):
    keyfs = KeyFS(gen_path(), storage, io_file_factory=storage_io_file_factory, schema=KeyFSSchema)
...
class TestKeyFS:
    ...
    def test_keyfs_readonly(self, storage, storage_io_file_factory, tmpdir):
        keyfs = KeyFS(
            tmpdir, storage, io_file_factory=storage_io_file_factory, readonly=True, schema=KeyFSSchema
        )
    ...
    @notransaction
    @pytest.mark.parametrize(("before", "after", "rotype"), [
...
class TestTransactionIsolation:
    ...
    def test_import_changes_subscriber(self, keyfs, storage, tmpdir):
        ...
        assert serial == 0
        assert changes == {
            new_keyfs.schema.NAME(name="world"): ({1: 1}, -1)
        }
    ...
    def test_import_changes_subscriber_error(
        ...
        new_keyfs = KeyFS(
            tmpdir.join("newkeyfs"), storage, io_file_factory=storage_io_file_factory, schema=KeyFSSchema
        )
    ...
    def test_cache_interference(self, storage, storage_io_file_factory, tmpdir):
        ...
        keyfs1 = KeyFS(
            tmpdir.join("keyfs1"), storage, io_file_factory=storage_io_file_factory, schema=KeyFSSchema
        )
...
class TestSubscriber:
    ...
    def test_notifications_retried_after_exception(
        ...
        def make_keyfs():
            keyfs = KeyFS(tmpdir, storage, io_file_factory=storage_io_file_factory, schema=KeyFSSchema)
            key1 = keyfs.schema.typedkey("NAME1", "hello", int, int)
