...
def get_pypi_project_names(testapp):
    ...
    key_project = stage.key_project.with_resolved_parent()
    return {key.name for key in key_project.iter_ulidkeys()}
...
def test_simple_refresh(xom, pypistage, testapp):
    ...
    with xom.keyfs.read_transaction():
        ...
        cache_info = (
            pypistage.key_remoteprojectinfo("hello").with_resolved_parent().get()
        )
    assert remotefiles == {}
    assert simpledata == {}
...
def test_upload_and_delete_project(mapp, testapp):
    ...
    with mapp.xom.keyfs.read_transaction():
        ...
        assert not list(
            stage.key_simpledata("pkg1").with_resolved_parent().iter_ulidkeys()
        )
...
def test_delete_remote(mapp, monkeypatch, simpypi, testapp, xom):
    ...
    with testapp.xom.keyfs.read_transaction():
        ...
        key_project = stage.key_project.with_resolved_parent()
        assert {x.name for x in key_project.iter_ulidkeys()} == {name}
        assert getentry(testapp, path).file_exists()
    # remove
    ...
    with testapp.xom.keyfs.read_transaction():
        stage = testapp.xom.model.getstage(api.stagename)
        key_project = stage.key_project.with_resolved_parent()
        assert {x.name for x in key_project.iter_ulidkeys()} == set()
