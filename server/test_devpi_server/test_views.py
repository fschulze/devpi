...
def test_simple_refresh(xom, pypistage, testapp):
    ...
    with xom.keyfs.read_transaction():
        ...
        projects = {
            ...
            for k, v in key_mirrorfile.iter_ulidkey_values()}
        cache_info = pypistage.key_projectcacheinfo("hello").get()
    assert projects == {'hello-1.0.zip': dict(
...
def test_simple_refresh_inherited(mapp, xom, pypistage, testapp, project,
    ...
    with xom.keyfs.read_transaction():
        ...
        info = list(key_mirrorfile.iter_ulidkeys())
    assert bool(info)
    ...
    with xom.keyfs.read_transaction():
        ...
        info = list(key_mirrorfile.iter_ulidkeys())
    assert len(info) == 1
    assert info[0].name.endswith("-2.0.zip")
...
class TestSubmitValidation:
    ...
    def test_upload_delete_upload_again(self, mapp):
        ...
        content = b"content"
        mapp.upload_file_pypi("pkg-1.1.whl", content, "pkg", "1.1", register=False, set_whitelist=False)
        mapp.upload_file_pypi("pkg-1.1.tar.gz", content, "pkg", "1.1", register=False, set_whitelist=False)
        info = mapp.getjson("/%s/pkg" % mapp.api.stagename)
        links = [
            x["href"]
            for x in info["result"]["1.1"]["+links"]]
        assert len(links) == 2
...
def test_submit_authorization(mapp, testapp):
    ...
    headers = {'Authorization': (
        b"Basic " + b64encode(
            f'{api.user}:{api.password}'.encode("ascii"))).decode('ascii')}
...
@proj
def test_upload_and_push_internal_docs_only(mapp, testapp, proj):
    mapp.create_user("user1", "1")
    mapp.create_and_login_user("user2")
    mapp.create_index("prod", indexconfig=dict(acl_upload=["user1", "user2"]))
    mapp.create_index("dev", indexconfig=dict(acl_upload=["user2"]))
