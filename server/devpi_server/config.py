...
def add_storage_options(parser, pluginmanager):
    ...
    backends = sorted(
        (s for s in storages if not s.hidden),
        key=attrgetter("name"))
    parser.addoption(
        ...
        help="the storage backend to use.\n" + ", ".join(
             f'"{x.name}": {x.description}' for x in backends))
...
class Config:
    ...
    def _determine_role(self):
        ...
        if role not in ("auto", "primary", "replica", "standalone"):
            raise Fatal(
                f"configuration error, unknown role {role!r}")
    ...
    def _storage_info_from_name(self, name, settings):
        from .main import Fatal
        storages = self.pluginmanager.hook.devpiserver_describe_storage_backend(settings=settings)
    ...
    def _determine_storage(self):
        ...
        self._storage_info = dict(
            name=storage_info.name,
            settings=settings)

    def storage_exists(self):
        return self.storage_info.exists(
            self.server_path, self.storage_info.settings)
