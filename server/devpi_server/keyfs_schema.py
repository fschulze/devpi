...
class PTypedKeyDescriptor(Generic[KeyType, KeyTypeRO]):
    ...
    def __get__(self, instance: KeyFSSchema, owner: type[KeyFSSchema] | None = None) -> PTypedKey[KeyType, KeyTypeRO]:
        key: PTypedKey[KeyType, KeyTypeRO] = PTypedKey(instance.keyfs, self.path, self.key_type, self.name)
        # cache attribute
...
class TypedKeyDescriptor(Generic[KeyType, KeyTypeRO]):
    ...
    def __get__(self, instance: KeyFSSchema, owner: type[KeyFSSchema]) -> TypedKey[KeyType, KeyTypeRO]:
        key: TypedKey[KeyType, KeyTypeRO] = TypedKey(instance.keyfs, RelPath(self.path), self.key_type, self.name)
        # cache attribute
        instance.__dict__[self.name] = key
        return key
...
class KeyFSSchema(metaclass=KeyFSSchemaMeta):
    ...
    def __iter__(self) -> Iterator[PTypedKey | TypedKey]:
        for _name, key in getmembers(self, lambda x: isinstance(x, (PTypedKey, TypedKey))):
            yield key
    ...
    def ptypedkey(
        ...
        setattr(self, name, PTypedKeyDescriptor(name, path, key_type, key_rotype).__get__(self, self.__class__))
    ...
    def typedkey(
        ...
        assert not hasattr(self, name)
        setattr(self, name, TypedKeyDescriptor(name, path, key_type, key_rotype).__get__(self, self.__class__))
        return getattr(self, name)
