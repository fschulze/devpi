...
class KeyFSSchemaMeta(type):
    ...
    def decl_named_key(
        ...
        return NamedKeyDescriptor(key_name, pattern_or_name, parent_key, key_type, key_rotype)
    ...
    def decl_named_key_factory(
        ...
        return NamedKeyFactoryDescriptor(key_name, pattern_or_name, parent_key, key_type, key_rotype)
...
class LocatedKeyDescriptor(Generic[KeyType, KeyTypeRO]):
    ...
    def __get__(self, instance: KeyFSSchema, owner: type[KeyFSSchema] | None = None) -> LocatedKey[KeyType, KeyTypeRO]:
        key: LocatedKey[KeyType, KeyTypeRO] = LocatedKey(instance.keyfs, self.key_name, self.location, self.name, self.key_type)
        # cache attribute
...
class NamedKeyDescriptor(Generic[KeyType, KeyTypeRO]):
    ...
    def __init__(
        ...
        key_name: str,
        pattern_or_name: str,
        parent_key: NamedKey | NamedKeyDescriptor | NamedKeyFactory | NamedKeyFactoryDescriptor,
        key_type: type[KeyType],
        key_rotype: type[KeyTypeRO],
    ) -> None:
    ...
    def __get__(self, instance: KeyFSSchema, owner: type[KeyFSSchema]) -> NamedKey[KeyType, KeyTypeRO]:
        ...
        if isinstance(parent_key, NamedKeyFactoryDescriptor):
            parent_key = parent_key.__get__(instance, owner)
        key: NamedKey[KeyType, KeyTypeRO] = NamedKey(instance.keyfs, self.key_name, self.pattern_or_name, parent_key, self.key_type)
        # cache attribute
        instance.__dict__[self.key_name] = key
        return key
...
class NamedKeyFactoryDescriptor(Generic[KeyType, KeyTypeRO]):
    ...
    def __init__(
        ...
        key_name: str,
        pattern_or_name: str,
        parent_key: NamedKey | NamedKeyDescriptor | NamedKeyFactory | NamedKeyFactoryDescriptor | None,
        key_type: type[KeyType],
        key_rotype: type[KeyTypeRO],
    ) -> None:
    ...
    def __get__(self, instance: KeyFSSchema, owner: type[KeyFSSchema]) -> NamedKeyFactory[KeyType, KeyTypeRO]:
        parent_key = self.parent_key
        if isinstance(parent_key, NamedKeyDescriptor):
            parent_key = parent_key.__get__(instance, owner)
        if isinstance(parent_key, NamedKeyFactoryDescriptor):
            parent_key = parent_key.__get__(instance, owner)
        key: NamedKeyFactory[KeyType, KeyTypeRO] = NamedKeyFactory(instance.keyfs, self.key_name, self.pattern_or_name, parent_key, self.key_type)
        # cache attribute
        instance.__dict__[self.key_name] = key
        return key
...
class KeyFSSchema(metaclass=KeyFSSchemaMeta):
    ...
    def __iter__(self) -> Iterator[LocatedKey | NamedKey]:
        for _name, key in getmembers(self, lambda x: isinstance(x, (LocatedKey, NamedKey))):
            yield key
    ...
    def located_key(
        ...
        setattr(self, key_name, LocatedKeyDescriptor(key_name, location, name, key_type, key_rotype).__get__(self, self.__class__))
    ...
    def named_key(
        ...
        assert not hasattr(self, key_name)
        setattr(self, key_name, NamedKeyDescriptor(key_name, pattern_or_name, parent_key, key_type, key_rotype).__get__(self, self.__class__))
        return getattr(self, key_name)
    ...
    def named_key_factory(
        ...
        assert not hasattr(self, key_name)
        setattr(self, key_name, NamedKeyFactoryDescriptor(key_name, pattern_or_name, parent_key, key_type, key_rotype).__get__(self, self.__class__))
        return getattr(self, key_name)
