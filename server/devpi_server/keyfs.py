...
class Transaction:
    ...
    def resolve(self, key: LocatedKey, *, fetch: bool) -> ULIDKey:
        ...
        if fetch:
            fetched_keydata = next(
                self.conn.iter_keys_at_serial((key,), at_serial=self.at_serial, with_deleted=True),
                absent)
            ...
            fetched_ulid_key = next(
                self.conn.iter_ulidkeys_at_serial((key,), at_serial=self.at_serial, with_deleted=True),
                absent)
    ...
    def resolve_at(self, key: LocatedKey, at_serial: int) -> ULIDKey:
        fetched_ulid_key: ULIDKey | Absent = next(
            self.conn.iter_ulidkeys_at_serial((key,), at_serial=at_serial, with_deleted=True),
            absent)
        if isinstance(fetched_ulid_key, Absent):
            raise KeyError(key)
        return fetched_ulid_key
