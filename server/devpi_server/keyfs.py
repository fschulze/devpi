    ...
    def import_changes(self, serial: int, changes: Sequence[KeyData]) -> None:
        ...
        with self.get_connection(write=False) as conn:
            ...
            for key_names in self.keynames_by_parent:
                ...
                if not keys:
                    continue
                for kd in conn.iter_keys_at_serial(keys, serial - 1, fill_cache=True, with_deleted=True):
                    old_keys[kd.key] = kd
            records: list[Record] = []
            for change in changes:
        ...
        with contextlib.ExitStack() as cstack:
            ...
            for record in records:
                ...
                subscriber_changes[record.key] = (
                    deleted if record.value is deleted else ensure_deeply_readonly(record.value),
                    record.back_serial)
