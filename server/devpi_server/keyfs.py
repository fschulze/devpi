...
class KeysChecker:
    ...
    def iter_ulidkey_values(self, *, with_deleted):
        ...
        for ulid_key in self._dirty:
            ...
            if (
                ...
                if with_deleted or value is not deleted:
        ...
        for ulid_key in self._original:
            if (
                ...
                continue
            processed_ulid_keys.add(ulid_key)
            (_back_serial, old_ulid_key, old_value) = self._original[ulid_key]
...
class Transaction:
    ...
    def back_serial(self, key: LocatedKey | ULIDKey) -> int:
        if not isinstance(key, ULIDKey):
            try:
                ...
                return -1
        (back_serial, _ulid_key, _val) = self.get_original(key)
        return back_serial
    ...
    def last_serial(self, key: LocatedKey | ULIDKey) -> int:
        ...
        (last_serial, _ulid_key, _val) = self.get_original(key)
        return last_serial
    ...
    @overload
    def get(self, key: LocatedKey, default: Absent = absent) -> KeyFSTypesRO: ...

    @overload
    def get(self, key: ULIDKey, default: Absent = absent) -> KeyFSTypesRO: ...

    def get(self, key: LocatedKey | ULIDKey, default: Any = absent) -> Any:
        """Return current read-only value referenced by key."""
        (_ulid_key, val) = self._get(key)
    ...
    def exists(self, key: LocatedKey | ULIDKey) -> bool:
        ...
        if key in self._dirty:
            ...
            (_serial, _ulid_key, val) = self.get_original(key)
    ...
    def deleted(self, key: LocatedKey | ULIDKey) -> bool:
        if not isinstance(key, ULIDKey):
            try:
                key = self.resolve(key, fetch=True)
            except KeyError:
