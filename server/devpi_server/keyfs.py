...
class TxNotificationThread:
    ...
    def on_key_change(self, key, subscriber):
        ...
        if not any('KeyChangeEvent' in str(v) for v in get_annotations(subscriber).values()):
            msg = f"The event subscriber {subscriber!r} has no KeyChangeEvent annotation"
    ...
    def get_ixconfig(self, entry: FileEntry, at_serial: int) -> dict[str, object] | None:
        user = entry.user
        index = entry.index
    ...
    def skip_by_index_config(self, ixconfig: dict[str, object] | None) -> bool:
        ...
        return ixconfig.get('type') == 'mirror' and bool(ixconfig.get('mirror_use_external_urls', False))
    ...
    def check_file_change(self, change: KeyData, event_serial: int) -> None:
        ...
        if entry.deleted_or_never_fetched or self.skip_by_index_config(self.get_ixconfig(entry, event_serial)):
        ...
        if event_serial < serial:
            ...
            if self.skip_by_index_config(current_ixconfig):
                return
            if current_ixconfig and current_ixconfig.get('type') == 'mirror' and current_entry.project is None:
        ...
        missing_source = missing_source if missing_source else f"entry.meta {entry.meta!r}"
        ...
        for change in changes:
            ...
            for sub in subscribers:
                ...
                self.log.debug(
                    ...
                    subname, change.key, change, event_serial)
                try:
                    sub(ev)
                except Exception:
                    if raising:
                        raise
                    self.log.exception("calling %s failed, serial=%s", sub, event_serial)
...
class KeyFS:
    ...
    def __init__(self, basedir: Path, storage_info: StorageInfo, *, io_file_factory: Callable | None = None, readonly: bool = False) -> None:
    ...
    def import_changes(self, serial: int, changes: Sequence[KeyData]) -> None:
        with contextlib.ExitStack() as cstack:
            ...
            io_file = (
                None
                if self.io_file_factory is None else
                cstack.enter_context(self.io_file_factory(conn)))
    ...
    def register_anonymous_key(self, key_name: str, parent_key: PatternedKey | None, key_type: type[KeyFSTypes]) -> LocatedKey | PatternedKey:
...
class KeysChecker:
    ...
    def iter_ulidkey_values(self):
        ...
        for ulid_key in self._dirty:
            if ulid_key in processed_ulid_keys or ulid_key.key_name not in all_key_names:
                continue
            processed_ulid_keys.add(ulid_key)
            value = self._dirty[ulid_key]
            if isinstance(value, Deleted):
                continue
            if ulid_key.key_name in key_names or (ulid_key.key_name, ulid_key.location) in key_name_locations or (ulid_key.key_name, ulid_key.relpath) in key_name_relpaths:
        ...
        for ulid_key in self._original:
            ...
            if isinstance(old_ulid_key, Absent) or isinstance(old_value, Absent):
                continue
            if old_ulid_key.key_name in key_names or (old_ulid_key.key_name, old_ulid_key.location) in key_name_locations or (old_ulid_key.key_name, old_ulid_key.relpath) in key_name_relpaths:
    ...
    def iter_ulidkeys(self):
        ...
        for ulid_key in self._ulid_keys.values():
            if isinstance(ulid_key, (Absent, Deleted)) or ulid_key in processed_ulid_keys or ulid_key.key_name not in all_key_names:
                continue
            processed_ulid_keys.add(ulid_key)
...
class Transaction:
    ...
    def iter_keys_at_serial(self, keys: LocatedKey, at_serial: int) -> Iterator[KeyData]:
        return self.conn.iter_keys_at_serial(keys, at_serial, fill_cache=False, with_deleted=True)

    def iter_serial_and_value_backwards(self, key: LocatedKey | ULIDKey, last_serial: int) -> Iterator[tuple[int, KeyFSTypesRO]]:
    ...
    def iter_ulidkey_values_for(self, keys: Sequence[LocatedKey | PatternedKey | SearchKey], *, fill_cache: bool = True, with_deleted: bool = False) -> Iterator[tuple[ULIDKey, KeyFSTypesRO]]:
        ...
        for keydata in self.conn.iter_keys_at_serial(keys, self.at_serial, skip_ulid_keys=keys_checker.ulidkeys_to_skip, fill_cache=fill_cache, with_deleted=with_deleted):
            key = keydata.key
    ...
    def iter_ulidkeys_for(self, keys: Sequence[LocatedKey | PatternedKey | SearchKey], *, fill_cache: bool = True, with_deleted: bool = False) -> Iterator[ULIDKey]:
        keys_checker = KeysChecker(self, keys)
        yield from keys_checker.iter_ulidkeys()
        if all(k in self._got_all for k in keys):
            return
        processed_ulid_keys = keys_checker.processed_ulid_keys
        for key in self.conn.iter_ulidkeys_at_serial(keys, self.at_serial, skip_ulid_keys=keys_checker.ulidkeys_to_skip, fill_cache=fill_cache, with_deleted=with_deleted):
            if key in processed_ulid_keys:
                continue
            _key = self.keyfs._keys[key.key_name]
            if not isinstance(_key, LocatedKey):
    ...
    def get_last_serial_and_value_at(self, key: LocatedKey | ULIDKey, at_serial: int) -> tuple[int, ULIDKey | Absent, KeyFSTypesRO | Absent]:
        if not isinstance(key, ULIDKey):
            try:
    ...
    def last_serial_and_value_at(self, key: LocatedKey | ULIDKey, at_serial: int) -> tuple[int, KeyFSTypesRO]:
        if not isinstance(key, ULIDKey):
            key = self.resolve_at(key, at_serial)
        data = self.conn.get_key_at_serial(key, at_serial)
    ...
    def get_original(self, key: LocatedKey | ULIDKey) -> tuple[int, ULIDKey | Absent, KeyFSTypesRO | Absent]:
        """ Return original value from start of transaction,
            without changes from current transaction."""
        if not isinstance(key, ULIDKey):
            key = self.resolve(key, fetch=True)
        if key not in self._original:
            (serial, ulid_key, val) = self.get_last_serial_and_value_at(key, self.at_serial)
    ...
    def _get(self, key: LocatedKey | ULIDKey) -> tuple[ULIDKey | Absent, KeyFSTypes | KeyFSTypesRO | Absent | Deleted]:
    ...
    def resolve(self, key: LocatedKey, *, fetch: bool) -> ULIDKey:
        ...
        (_key, ulid_key) = (key, absent) if possible_ulid_key is None else possible_ulid_key
    ...
    def resolve_at(self, key: LocatedKey, at_serial: int) -> ULIDKey:
        result = self.conn.iter_ulidkeys_at_serial((key,), at_serial=at_serial, fill_cache=False, with_deleted=True)
    ...
    def resolve_keys(self, keys: Iterable[LocatedKey], *, fetch: bool, fill_cache: bool = False, new_for_missing: bool) -> Iterator[tuple[LocatedKey, ULIDKey | Absent | Deleted]]:  # noqa: PLR0912
        missing = set()
        for key in keys:
            ...
            yield (
                ...
                if _ulid_key is absent and new_for_missing else
                (key, _ulid_key))
        if not missing:
            return
        processed = set()
        if fetch:
            for keydata in self.conn.iter_keys_at_serial(missing, at_serial=self.at_serial, fill_cache=fill_cache, with_deleted=True):
                ulid_key = keydata.key
                _key = self.keyfs._keys[ulid_key.key_name]
                if not isinstance(_key, LocatedKey):
                    _key = _key(**ulid_key.params)
                if (parent_key := ulid_key.parent_key) is not None and not self.exists(parent_key):
                    # parent was deleted
                    ulid_key = deleted
                else:
                    self._original[ulid_key] = (keydata.last_serial, ulid_key, keydata.value)
                self._ulid_keys[_key] = ulid_key
                processed.add(_key)
                yield (_key, ulid_key)
        else:
            for ulid_key in self.conn.iter_ulidkeys_at_serial(missing, at_serial=self.at_serial, fill_cache=fill_cache, with_deleted=True):
                _key = self.keyfs._keys[ulid_key.key_name]
                if not isinstance(_key, LocatedKey):
                    _key = _key(**ulid_key.params)
                if (parent_key := ulid_key.parent_key) is not None and not parent_key.exists():
                    # parent was deleted
    ...
    def set(self, key: LocatedKey | ULIDKey, val: KeyFSTypes) -> None:
        if not isinstance(val, key.key_type) and not issubclass(key.key_type, type(val)):
            raise TypeError(
