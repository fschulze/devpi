...
class TxNotificationThread(Generic[Schema]):
    ...
    def on_key_change(
        self, key: LocatedKey | NamedKey | NamedKeyFactory | str, subscriber: Callable
    ) -> None:
    ...
    def get_ixconfig(self, entry: FileEntry, event_serial: int) -> dict | None:
        ...
        with self.keyfs.read_transaction():
            key = cast("NamedKeyFactory[dict, DictViewReadonly]", self.keyfs.schema.USER)(user=user)  # type: ignore[attr-defined]
            value = key.get_mutable()
            key = cast("NamedKeyFactory[dict, DictViewReadonly]", self.keyfs.schema.INDEX)(  # type: ignore[attr-defined]
        ...
        with self.get_connection() as conn:
            ...
            def iter_file_path_infos(
                ...
                for relpath in relpaths:
                    if (
                        key := self.match_key(
                            relpath, self.schema.PYPIFILE_NOMD5, self.schema.STAGEFILE  # type: ignore[attr-defined]
                        )
                    ) is not None and isinstance(
                        val := conn.get_key_at_serial(key, serial).value,
...
class Transaction:
    cache: dict[LocatedKey, KeyFSTypesRO | Deleted]
    _model: TransactionRootModel | Absent
    _original: dict[LocatedKey, tuple[int, KeyFSTypesRO | Absent | Deleted]]
    ...
    def get_value_at(self, key: LocatedKey[KeyType, KeyTypeRO], at_serial: int) -> KeyTypeRO:
        return self.last_serial_and_value_at(key, at_serial)[1]

    def last_serial(self, key: LocatedKey[KeyType, KeyTypeRO]) -> int:
    ...
    def get_original(self, key: LocatedKey[KeyType, KeyTypeRO]) -> tuple[int, KeyTypeRO | Absent | Deleted]:
        """ Return original value from start of transaction,
            without changes from current transaction."""
        if key not in self._original:
            tup = self.get_last_serial_and_value_at(key, self.at_serial)
            # make mypy happy
            (serial, val) = tup
    ...
    def deleted(self, typedkey: LocatedKey[KeyType, KeyTypeRO]) -> bool:
        if typedkey in self.cache:
            val = self.cache[typedkey]
            return isinstance(val, Deleted)
        return isinstance(self.get_original(typedkey)[1], Deleted)
