...
class KeyFS:
    ...
    @overload
    def register_key(self, key: LocatedKey[T]) -> LocatedKey[T]: ...

    @overload
    def register_key(self, key: NamedKey[T]) -> NamedKey[T]: ...

    @overload
    def register_key(self, key: NamedKeyFactory[T]) -> NamedKeyFactory[T]: ...

    def register_key(
        self, key: LocatedKey[T] | NamedKey[T] | NamedKeyFactory[T]
    ...
    def register_named_key(
        ...
        key: LocatedKey[T] | NamedKey[T] | NamedKeyFactory[T]
...
class Transaction:
    ...
    def get_value_at(self, typedkey: LocatedKey, at_serial: int) -> KeyFSTypesRO | None:
        (_last_serial, val) = self.last_serial_and_value_at(typedkey, at_serial)
        return val
