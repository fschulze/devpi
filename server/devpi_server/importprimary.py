    ...
    def check(self, records):
        ...
        for record in records:
            ...
            if name not in to_keep:
                ...
                to_remove.add(name)
                to_keep.remove(name)
        assert self.value == to_add | to_keep, (self.keyname, self.value, self.existing, to_add, to_keep, to_remove)
        assert to_add == set(self.value).difference(self.existing), (self.keyname, self.value, self.existing, to_add, to_keep, to_remove)
        assert to_keep == set(self.existing).intersection(self.value), (self.keyname, self.value, self.existing, to_add, to_keep, to_remove)
        assert to_remove == set(self.existing).difference(self.value), (self.keyname, self.value, self.existing, to_add, to_keep, to_remove)
...
class ReplicaImport:
    ...
    def iter_migrated_changes(self, changes: Any) -> Iterable[tuple[str, str, int, Any]]:  # noqa: PLR0912
        sort_order = dict(
            USERLIST=1,
            USER=2,
            MIRRORNAMESINIT=3,
    ...
    def iter_parents(self, key, params):
        ...
        while parent_key is not None:
            yield (
                parent_key(**params)
                if isinstance(parent_key, PatternedKey) else
                parent_key)
    ...
    def iter_keys_to_fetch(self, changes: Any) -> Iterable[LocatedKey]:  # noqa: PLR0912
        ...
        for relpath, keyname, _back_serial, val in changes:
            if keyname in keyfs._keys:
                ...
                if (existing := self.recent_existing.get(("PROJECTNAME", relpath), None)) is not None:
                ...
                if (existing := self.recent_existing.get(("MIRRORFILE", relpath), None)) is not None:
                    missing = missing.difference(existing)
    ...
    def get_keys(self, keys_to_fetch: Iterable[LocatedKey]) -> dict[ULIDKey, KeyData]:
        ...
        for key_names in keynames_by_parent:
            ...
            if not _keys:
                continue
            keys.update(dict(tx.resolve_keys(
    ...
    def make_records(self, serial: int, changes: Any) -> Sequence[Record]:
        ...
        for relpath, keyname, _back_serial, _val in changes:
            ...
            if keyname == "PROJNAMES":
                ...
                if (existing := self.recent_existing.get(nameslist.key, None)) is not None:
