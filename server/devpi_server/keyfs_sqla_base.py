...
class BaseConnection:
    ...
    def _db_write_typedkeys(self, deleted_typedkeys: Sequence[dict], new_typedkeys: Sequence[dict], updated_typedkeys: Sequence[dict]) -> None:
        if deleted_typedkeys:
            self._sqlaconn.execute(
                sa.update(self.ulid_info_table)
                .where(
                    self.ulid_info_table.c.ulid == sa.bindparam("b_ulid")),
                deleted_typedkeys)
        if new_typedkeys:
            self._sqlaconn.execute(
                sa.insert(self.ulid_info_table),
                new_typedkeys)
            self._sqlaconn.execute(
                sa.insert(self.ulid_latest_serial_table),
                [dict(latest_serial=x["added_at_serial"], ulid=x["ulid"]) for x in new_typedkeys])
    ...
    def _ulidkey_for_row(self, row: sa.Row, serial: int) -> ULIDKey:
        ...
        if parent_ulid:
            parent_key = self.get_ulid_at_serial(parent_ulid, serial, parent_serial=row.latest_parent_serial).key
    ...
    def get_ulid_at_serial(self, ulid: ULID, serial: int, *, parent_serial: int = -1) -> KeyData:
        ...
        if result is absent:
            ...
            results = list(self._iter_relpaths_at(
                relpaths_ulid_serials_stmt,
                fetch_serial, with_deleted=True))
            if not results:
                raise KeyError(ulid)
            (result,) = results
    ...
    def _write_records(self, serial: int, records: Sequence[Record], renames: Sequence[str]) -> None:
        execute = self._sqlaconn.execute
        threadlog.debug("writing changelog for serial %s", serial)
        ulid_changelog = [
            dict(
                ulid=int(record.key.ulid),
                serial=serial, back_serial=record.back_serial,
                value=None if record.value is deleted else dumps(record.value))
            for record in records]
        if ulid_changelog:
            execute(
                sa.insert(self.ulid_changelog_table)
                .values(ulid_changelog))
        execute(
            sa.insert(self.renames_table)
...
class Writer:
    ...
    def commit(self, commit_serial: int) -> LazyChangesFormatter:
            ...
            if record.key != record.old_key:
                ...
                if record.value is deleted:
                    deleted_typedkeys.append(dict(
                        b_ulid=int(record.key.ulid),
                        deleted_at_serial=commit_serial if record.value is deleted else None))
        self.conn._db_write_typedkeys(deleted_typedkeys, new_typedkeys, updated_typedkeys)
        ...
        analyze_frequency = (
            ...
            10000)
        if (
            ((commit_serial % analyze_frequency) == 0) or len(records) > 10000
        ):
            self.conn.analyze()
        self.conn.commit()
        self.storage.last_commit_timestamp = time.time()
        return LazyChangesFormatter({(x.key.key_name, x.key.name, x.key.parent_ulid) for x in records}, files_commit, files_del)
