...
class BaseConnection:
    ...
    def db_read_typedkey(self, relpath: RelPath) -> tuple[str, int]:
        latest_serial_stmt = (
            ...
            .where(self.relpath_ulid_table.c.relpath == relpath)
            .group_by(self.relpath_ulid_table.c.relpath)
        )
        latest_serial_sq = latest_serial_stmt.subquery("latest_serial_sq")
        stmt = sa.select(
            self.relpath_ulid_table.c.keytype, latest_serial_sq.c.serial
        ).join(
            latest_serial_sq,
            sa.and_(
...
class Writer:
    ...
    def new_ulid(self) -> ULID:
        ...
        _current_ulid_ts = storage._current_ulid_ts
        _used_ulids = storage._used_ulids
        while 1:
            ulid = ULID.new()
            if ulid not in _used_ulids:
                if (ts_part := ulid.ts_part) != _current_ulid_ts:
                    _used_ulids.clear()
                    _current_ulid_ts = storage._current_ulid_ts = ts_part
