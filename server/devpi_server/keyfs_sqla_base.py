...
class BaseConnection:
        ...
        if updated_typedkeys:
            ...
            if self._sqlaconn.dialect.update_executemany_returning:
                ulid_serials = [
                    ...
                    for x in self._sqlaconn.execute(relpath_update_stmt, updated_typedkeys)]
            else:
                ulid_serials = []
                for updated_typedkey in updated_typedkeys:
                    result = self._sqlaconn.execute(relpath_update_stmt, updated_typedkey).fetchall()
                    if not result:
                        raise RuntimeError
                    ulid_serials.extend(
                        dict(b_ulid=x.ulid, b_serial=x.serial, b_back_serial=x.b_back_serial)
            ...
            ulid_update_stmt = (
                ...
                .where(
                    ...
                    self.ulid_latest_serial_table.c.serial == sa.bindparam("b_back_serial"))
                .values(serial=sa.bindparam("b_serial")))
    ...
    def get_raw_changelog_entry(self, serial: int) -> bytes | None:
        ...
        renames = loads(data)
        changes = [
            (c.keyname, c.relpath, c.back_serial, None if c.value is deleted else c.value)
    ...
    def _iter_relpaths_at(self, relpaths, serial):
        execute = self._sqlaconn.execute
        relpaths_stmt = (
            sa.select(self.relpath_ulid_table)
            .where(
                sa.tuple_(
                    self.relpath_ulid_table.c.relpath,
                    self.relpath_ulid_table.c.keytype,
    ...
    def get_key_at_serial(self, key: LocatedKey, serial: int) -> KeyData:
        ...
        if result is absent:
            results = list(self._iter_relpaths_at((sa.tuple_(key.relpath, key.key_name),), serial))
            if not results:
                raise KeyError(key)
            (result,) = results
        ...
        latest_serial_stmt = (
            ...
            .where(
                sa.tuple_(
                    ...
                    {sa.tuple_(x.key.relpath, x.key.key_name) for x in records}))
            .group_by(
                self.relpath_ulid_table.c.relpath,
                self.relpath_ulid_table.c.keytype,
            ))
