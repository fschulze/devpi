...
class Cache:
    ...
    def add_keydata(self, serial: int, keydata: KeyData) -> None:
        ...
        if keydata.value is not deleted and gettotalsizeof(keydata.value, maxlen=100000) is None:
...
class ULIDKeyChecker:
    ...
    def add(self, key: ULIDKey) -> None:
        ...
        elif (skey := self.key_name_locations.get((key.key_name, key.location))) is not None:
            cache_key = self.cache_keys[skey]
        elif (lkey := self.key_name_relpaths.get((key.key_name, key.relpath))) is not None:
    ...
    def got_all(self) -> bool:
        return all(bool(self.got_all_by_cache_key.get(x)) for x in self.cache_keys.values())
...
class BaseConnection:
    ...
    def _dump(self) -> None:
        ...
        for table in (self.ulid_latest_serial_table, self.ulid_info_table, self.ulid_changelog_table):
    ...
    def _ppexplain(self, stmt: sa.Executable) -> None:
        text = str(explain(stmt, analyze=True).compile(
            compile_kwargs=dict(literal_binds=True),
            dialect=self._sqlaconn.dialect))
        results = self._sqlaconn.connection.execute(text).fetchall()  # type: ignore[attr-defined]
    ...
    def _ppsql(self, stmt: sa.Executable) -> None:
        ...
        text = str(stmt.compile(  # type: ignore[attr-defined]
            compile_kwargs=dict(literal_binds=True),
            dialect=self._sqlaconn.dialect))
    ...
    def db_read_last_changelog_serial(self) -> int:
        result = self._sqlaconn.execute(sa.select(sa.func.coalesce(
            sa.func.max(self.renames_table.c.serial),
            -1))).scalar()
    ...
    def last_key_serial(self, key: LocatedKey) -> int:
        stmt = (
            sa.select(
                sa.func.max(self.ulid_latest_serial_table.c.latest_serial))
            .join(
    ...
    def iter_changes_at(self, serial: int) -> Iterator[KeyData]:
        ...
        relpaths_ulid_serials_stmt = self._relpaths_ulid_serials_stmt(
            ...
            serial=serial, with_deleted=True)
        parent_key_names = self.storage.parent_key_names
        for keydata in self._iter_relpaths_at(relpaths_ulid_serials_stmt, serial, with_deleted=True):
    ...
    def get_raw_changelog_entry(self, serial: int) -> bytes | None:
        data = self._sqlaconn.execute(
            sa.select(self.renames_table.c.data)
            .where(self.renames_table.c.serial == serial)).scalar()
        if data is None:
            return None
        renames = loads(data)
        changes = [
            (
                c.key.key_name, c.key.relpath,
                int(c.key.ulid), None if c.key.parent_ulid is None else int(c.key.parent_ulid),
                c.back_serial, None if c.value is deleted else c.value)
    ...
    def _relpaths_ulid_serials_stmt(self, *whereclause: sa._ColumnExpressionArgument[bool], serial: int, with_deleted: bool) -> sa.Executable:
        full_relpaths_ulid_serials_stmt = (
            sa.select(
                self.ulid_info_table)
            .where(
                *whereclause,
                self.ulid_info_table.c.added_at_serial <= serial))
        if not with_deleted:
        ...
        full_relpaths_ulid_serials_cte = full_relpaths_ulid_serials_stmt.cte("full_relpaths_ulid_serials_cte")
        relpath_max_added_at_stmt = (
            sa.select(
                ...
                sa.func.max(full_relpaths_ulid_serials_cte.c.added_at_serial).label("max_added_at_serial"))
            .group_by(
                full_relpaths_ulid_serials_cte.c.keytype,
                full_relpaths_ulid_serials_cte.c.name,
                full_relpaths_ulid_serials_cte.c.parent_ulid))
        relpath_max_added_at_sq = relpath_max_added_at_stmt.subquery("relpath_max_added_at_sq")
        parent_ulid_stmt = (
            sa.select(
            ...
            .join_from(
                ...
                self.ulid_changelog_table,
                full_relpaths_ulid_serials_cte.c.parent_ulid == self.ulid_changelog_table.c.ulid))
        parent_ulid_sq = parent_ulid_stmt.subquery("parent_ulid_sq")
        parent_ulid_latest_serial_stmt = (
            sa.select(
                parent_ulid_sq.c.parent_ulid,
                sa.func.max(self.ulid_changelog_table.c.serial).label("latest_parent_serial"))
            .join_from(
                parent_ulid_sq,
                self.ulid_changelog_table,
        ...
        parent_ulid_latest_serial_sq = parent_ulid_latest_serial_stmt.subquery("parent_ulid_latest_serial_sq")
        return (
            sa.select(
                full_relpaths_ulid_serials_cte.c.keytype,
                full_relpaths_ulid_serials_cte.c.name,
            ...
            .join(
                ...
                sa.and_(
                    ...
                    full_relpaths_ulid_serials_cte.c.added_at_serial == relpath_max_added_at_sq.c.max_added_at_serial))
            .outerjoin(
                parent_ulid_latest_serial_sq,
                full_relpaths_ulid_serials_cte.c.parent_ulid == parent_ulid_latest_serial_sq.c.parent_ulid))

    def _iter_relpaths_at(self, relpaths_ulid_serials_stmt: Any, serial: int, *, with_deleted: bool) -> Iterator[KeyData]:
        execute = self._sqlaconn.execute
        relpaths_ulid_serials_cte = relpaths_ulid_serials_stmt.cte("relpaths_ulid_serials_cte")
        ulid_max_serial_stmt = (
            sa.select(
                self.ulid_changelog_table.c.ulid,
                sa.func.max(self.ulid_changelog_table.c.serial).label("ulid_max_serial"))
            .join(
                relpaths_ulid_serials_cte,
                sa.and_(
                    self.ulid_changelog_table.c.ulid == relpaths_ulid_serials_cte.c.ulid,
                    self.ulid_changelog_table.c.serial <= serial))
        ...
        ulid_changelog_stmt = (
            ...
            .join(
                ...
                sa.and_(
                    ...
                    self.ulid_changelog_table.c.serial == ulid_max_serial_sq.c.ulid_max_serial)))
        if not with_deleted:
            ulid_changelog_stmt = ulid_changelog_stmt.where(self.ulid_changelog_table.c.value.isnot(None))
        for row in execute(ulid_changelog_stmt).all():
            yield KeyData(
                key=self._ulidkey_for_row(row, serial), serial=row.serial, back_serial=row.back_serial,
                value=deleted if row.value is None else ensure_deeply_readonly(loads(row.value)))
    ...
    def _ulidkey_for_row(self, row: sa.Row, serial: int) -> ULIDKey:
        ...
        if parent_ulid:
            ...
            parent_key = self.get_ulid_at_serial(parent_ulid, serial, parent_serial=parent_serial).key
        ...
        if isinstance(key, PatternedKey):
            return key.make_ulid_key(location, row.name, ULID(row.ulid), parent_key=parent_key, parent_params=parent_params)
    ...
    def get_ulid_at_serial(self, ulid: ULID, serial: int, *, parent_serial: int = -1) -> KeyData:
        ...
        if result is absent:
            ...
            relpaths_ulid_serials_stmt = self._relpaths_ulid_serials_stmt(
                ...
                serial=fetch_serial, with_deleted=True)
            results = self._iter_relpaths_at(relpaths_ulid_serials_stmt, fetch_serial, with_deleted=True)
        ...
        *,
        ...
        for key in keys:
            ...
            if isinstance(key, LocatedKey):
                ...
                parent_ulid_keytype.add((
                    key.query_parent_ulid, key.key_name))
            elif isinstance(key, ULIDKey):
        ...
        is_large_query = ((
            len(keytype_name_parent_ulid)
            + len(parent_ulid_keytype)
            + len(key_ulid)) > MAX_SQL_VARIABLES)
        if keytype_name_parent_ulid:
            ...
            if is_large_query:
                ...
                execute(sa.schema.CreateTable(self.keytype_name_parent_ulid_table, if_not_exists=True))
                execute(self.keytype_name_parent_ulid_table.delete())
                while True:
                    keytype_name_parent_ulid_batch = list(islice(keytype_name_parent_ulid_iter, INSERT_BATCH_SIZE))
        ...
        if parent_ulid_keytype:
            ...
            if is_large_query:
                ...
                execute(sa.schema.CreateTable(self.parent_ulid_keytype_table, if_not_exists=True))
                execute(self.parent_ulid_keytype_table.delete())
                while True:
                    parent_ulid_keytype_batch = list(islice(parent_ulid_keytype_iter, INSERT_BATCH_SIZE))
            ...
            if is_large_query:
                ...
                while True:
                    ...
                    execute(
                        ...
                        [
                            dict(ulid=x)
                            for x in key_ulid_batch])
                    del key_ulid_batch
                key_ulid_in = sa.select(
                    self.key_ulid_table.c.ulid)
            else:
                key_ulid_in = (
                    sa.tuple_(
                        sa.literal(ulid),
        ...
        if skip_ulids:
            whereclauses.append(
                self.ulid_info_table.c.ulid.notin_(
                    int(x) for x in skip_ulids))
    ...
    def iter_keys_at_serial(
        ...
        *,
        ...
        checker = ULIDKeyChecker(
            self._cache, keys, at_serial, skip_ulid_keys,
            fill_cache=fill_cache, with_deleted=with_deleted)
        yield from checker.iter_keydata()
        if checker.got_all():
            return
        stmt = self._relpaths_ulid_serials_stmt_for_keys(
            keys, at_serial, skip_ulids=checker.ulids_to_skip(), with_deleted=with_deleted)
        for keydata in self._iter_relpaths_at(stmt, at_serial, with_deleted=with_deleted):
            yield keydata
            if fill_cache:
...
class Writer:
    ...
    def __init__(self, storage: BaseStorage, conn: BaseConnection, io_file: IIOFile | None) -> None:
        self.conn = conn
    ...
    def __exit__(self, cls: type[BaseException] | None, val: BaseException | None, tb: TracebackType | None) -> None:
    ...
    def commit(self, commit_serial: int) -> LazyChangesFormatter:  # noqa: PLR0912
        ...
        del self.records
        execute(
            sa.insert(self.conn.renames_table)
            .values((commit_serial, dumps(self.rel_renames))))
        ...
        if num_records > 20:
            ...
            def add_record_info(r: Record) -> None:
                records_key_info.add((r.key.key_name, r.key.name, r.key.parent_ulid, r.key.ulid))
        ...
        for batch_index in count(1):
            ...
            for record in batch:
                ...
                if record.value is deleted:
                    record_value = None
                    deleted_typedkeys.append(dict(
                        b_ulid=record_ulid,
                        b_deleted_at_serial=commit_serial))
                else:
                ...
                ulid_changelog.append(dict(
                    b_ulid=record_ulid,
                    b_serial=commit_serial,
                    b_back_serial=record_back_serial,
                    b_value=record_value))
            threadlog.debug("writing changelog batch %s/%s for serial %s", batch_index, num_batches, commit_serial)
            if deleted_typedkeys:
                execute(
                    sa.update(self.conn.ulid_info_table)
                    .where(
                        self.conn.ulid_info_table.c.ulid == sa.bindparam("b_ulid"))
                    .values(
            ...
            if new_typedkeys:
                execute(
                    sa.insert(self.conn.ulid_info_table)
                    .values(
                    ...
                    new_typedkeys)
                execute(
                    sa.insert(self.conn.ulid_latest_serial_table)
                    .values(
                        ulid=sa.bindparam("b_ulid"),
            ...
            if updated_typedkeys:
                execute(
                    ...
                    .where(
                        self.conn.ulid_latest_serial_table.c.ulid == sa.bindparam("b_ulid"))
                    .values(latest_serial=sa.bindparam("b_serial")),
                    updated_typedkeys)
            del updated_typedkeys
            execute(
                sa.insert(self.conn.ulid_changelog_table)
                .values(
                    ulid=sa.bindparam("b_ulid"),
            ...
            del ulid_changelog
            del batch
        (files_commit, files_del) = self.conn._write_dirty_files() if self.io_file else ([], [])
        analyze_frequency = (
            100
            if commit_serial < 1000 else
            1000
            if commit_serial < 20000 else
            10000)
        if (
            ((commit_serial % analyze_frequency) == 0) or num_records > 10000
        ):
            self.conn.analyze()
        self.conn.commit()
...
class BaseStorage:
    ...
    def define_tables(self, metadata_obj: sa.MetaData, binary_type: sa._Binary) -> dict:
        tables = dict(
            key_ulid_table=sa.Table(
                "key_ulid",
                metadata_obj,
...
def cache_metrics(storage: BaseStorage) -> list[tuple[str, str, object]]:
    ...
    if large_cache:
        result.extend([
            ('devpi_server_large_cache_evictions', 'counter', large_cache.evictions),
            ...
            ('devpi_server_large_cache_items', 'gauge', len(large_cache.data) if large_cache.data else 0)])
    if small_cache:
        result.extend([
            ('devpi_server_small_cache_evictions', 'counter', small_cache.evictions),
            ...
            ('devpi_server_small_cache_items', 'gauge', len(small_cache.data) if small_cache.data else 0)])
    return result
