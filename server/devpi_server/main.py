...
class XOM:
    ...
    def create_task(self, coroutine: Coroutine) -> asyncio.Task:
        task: asyncio.Task = asyncio.ensure_future(coroutine, loop=self.async_thread.loop)
    ...
    async def async_httpget(self, url, allow_redirects, timeout=None, extra_headers=None):
        warnings.warn(
            "The async_httpget method is deprecated, "
            "use http.async_get instead",
            DeprecationWarning,
            stacklevel=2)
    ...
    def httpget(self, url, allow_redirects, timeout=None, extra_headers=None):
        warnings.warn(
            "The httpget method is deprecated, "
...
def set_default_indexes(model):
    ...
    if model.xom.config.no_root_pypi:
        return
    if not root_user.key_index("pypi").exists():
