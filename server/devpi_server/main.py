...
class XOM:
    ...
    def __init__(self, config: Config, http: Any = None) -> None:
        self.config = config
...
_pypi_ixconfig_default = {
    ...
    "remote_web_url_fmt": "https://pypi.org/project/{name}/",
}
...
def tween_request_profiling(handler, registry):
    ...
    (_num_profile, *_profile_options) = [
        x.strip()
        for x in cast("str", registry["xom"].config.args.profile_requests).split(",")
    ]
