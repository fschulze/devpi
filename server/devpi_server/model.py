...
class IndexBases:
    ...
    def get_mergeable_stages(
        ...
        return self.get_project_inheritance_info(project).iter_stages(opname)
...
class BaseStage:
    ...
    def get_mirror_whitelist_info(
        ...
        return dict(
            has_mirror_base=project_inheritance_info.has_project_from_mirror,
            blocked_by_mirror_whitelist=project_inheritance_info.blocked_mirror_name,
        )
...
class PrivateStage(BaseStage):
    ...
    def _get_elink_dicts(
        ...
        return [el.linkdict for el in self._get_elinks(project, version, rel=rel)]
    ...
    def get_versiondata_perstage(
        ...
        *,
        ...
        if with_elinks:
            elinks = self._get_elink_dicts(project, version)
            if elinks:
                verdata = get_mutable_deepcopy(verdata)
...
class LinkStore:
    ...
    def get_links(
        ...
        def fil(elink):
            return (
                (not basename or basename == Path(elink.linkdict["relpath"]).name)
                and (
                    not entrypath
                    or entrypath
                    in (elink.linkdict["entrypath"], elink.linkdict["relpath"])
                )
                and (
                    not for_entrypath
                    or for_entrypath == elink.linkdict.get("for_entrypath")
                )
            )

        return [elink for elink in self._get_elinks(rel) if fil(elink)]
