...
class PrivateStage(BaseStage):
    ...
    def del_versiondata(self, project, version, cleanup=True):
        ...
        if cleanup:
            has_versions = next(self.key_version(project).iter_ulidkeys(), absent) is not absent
            if not has_versions:
    ...
    def get_simplelinks_perstage(self, project):
        version_keys = []
        versions = {}
        for k, v in self.key_version(project).iter_ulidkey_values():
            version_keys.append(self.key_versionfile(project=project, version=k.name))
            versions[k.ulid] = v
        links = self.SimpleLinks([])
        for k, v in self.keyfs.tx.iter_ulidkey_values_for(version_keys):
            version = versions[k.parent_ulid]
            href = v['entrypath']
            if (hash_spec := Digests(v['hashes']).best_available_spec):
                href += "#" + hash_spec
            links.append(SimplelinkMeta((
                k.name,
                href,
    ...
    def has_version_perstage(self, project: str, version: str) -> bool:
        return self.key_version(project, version).exists()
...
class LinkStore:
    ...
    @property
    def metadata(self):
        verdata = self.stage.get_versiondata_perstage(self.project, self.version, with_elinks=False)
...
def register_keys(xom, keyfs):
    ...
    keyfs.register_named_key("MIRRORFILE", "{filename}", project_key, dict)
    ...
    keyfs.notifier.on_key_change(keyfs.VERSION.key_name, sub.on_changed_version_config)
