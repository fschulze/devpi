...
class User:
    ...
    def key_index(self, index: str) -> LocatedKey[dict, DictViewReadonly]:
        return self.keyfs.schema.INDEX(
...
class PrivateStage(BaseStage):
    ...
    def key_projversions(self, project: NormalizedName | str) -> LocatedKey[set[str], SetViewReadonly[str]]:
...
class Schema(KeyFSSchema):
    ...
    USER = KeyFSSchema.decl_named_key_factory("USER", "{user}/.config", None, dict, DictViewReadonly)
    USERLIST = KeyFSSchema.decl_located_key("USERLIST", "", ".config", set, SetViewReadonly)
    INDEX = KeyFSSchema.decl_named_key_factory(
        "INDEX", "{index}/.config", USER, dict, DictViewReadonly
    )
    INDEXLIST = KeyFSSchema.decl_named_key("INDEXLIST", ".indexes", USER, set, SetViewReadonly)
    ...
    MIRRORNAMESINIT = KeyFSSchema.decl_named_key("MIRRORNAMESINIT", ".mirrornameschange", INDEX, int, int)

    # type "stage" related
    PROJSIMPLELINKS = KeyFSSchema.decl_named_key_factory(
        "PROJSIMPLELINKS", "{project}/.simple", INDEX, dict, DictViewReadonly
    )
    PROJVERSIONS = KeyFSSchema.decl_named_key("PROJVERSIONS", ".versions", PROJSIMPLELINKS, set, SetViewReadonly)
    PROJVERSION = KeyFSSchema.decl_named_key_factory(
        "PROJVERSION", "{version}/.config", PROJSIMPLELINKS, dict, DictViewReadonly
    )
    PROJNAMES = KeyFSSchema.decl_named_key("PROJNAMES", ".projects", INDEX, set, SetViewReadonly)
    VERSIONFILELIST = KeyFSSchema.decl_named_key("VERSIONFILELIST", ".files", PROJVERSION, set, SetViewReadonly)
    VERSIONFILE = KeyFSSchema.decl_named_key_factory("VERSIONFILE", "{filename}", PROJVERSION, dict, DictViewReadonly)
    STAGEFILE = KeyFSSchema.decl_named_key_factory(
        "STAGEFILE", "+f/{hashdir_a}/{hashdir_b}/{filename}", INDEX, dict, DictViewReadonly
    )

    # files related
    DIGESTPATHS = KeyFSSchema.decl_named_key_factory("DIGESTPATHS", "{digest}", None, set, SetViewReadonly)
