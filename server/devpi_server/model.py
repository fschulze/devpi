...
class PrivateStage(BaseStage):
    ...
    def _get_elinks(self, project, version):
        ...
        return [
            v
            for k, v in self.key_versionfile(project, version)
            .with_resolved_parent()
            .iter_ulidkey_values()
        ]
...
class LinkStore:
    ...
    def _get_elinks(self):
        return self.stage._get_elinks(self.project, self.version)
...
class MutableLinkStore(LinkStore):
    ...
    def _get_elinks(self):
        return get_mutable_deepcopy(super()._get_elinks())
    ...
    def _add_link_to_file_entry(
        ...
        if self.key_versionfile(file_entry.basename).exists(resolve_parents=True):
            raise RuntimeError
