...
class RootModel:
    ...
    def get_userlist(self):
        return [
            User(self, name)
            for name in self.keyfs.schema.USERLIST.get()
        ]
...
class User:
    ...
    def __init__(self, parent, name):
        ...
        self.key_indexes = self.keyfs.schema.INDEXLIST(
            user=self.name
        )
...
class BaseStage:
    ...
    def __init__(self, xom, username, index, ixconfig, customizer_cls):
        ...
        self.key_index = self.keyfs.schema.INDEX(
            user=username, index=index
        )
        self.key_projects = self.keyfs.schema.PROJNAMES(
            user=username, index=index
        )
...
class PrivateStage(BaseStage):
    ...
    def key_projversions(self, project: NormalizedName | str) -> TypedKey[set, SetViewReadonly]:
    ...
    def get_last_project_change_serial_perstage(self, project, at_serial=None):  # noqa: PLR0911, PLR0912
        ...
        for version in versions:
            ...
            if info is None:
                continue
            (version_serial, _version_info) = info
            ...
            for filename in versionfiles_info:
                ...
                if info is None:
                    continue
                (versionfile_serial, _versionfile_info) = info
...
class MutableLinkStore(LinkStore):
    ...
    def _add_link_to_file_entry(
        self, rel: Rel, file_entry: FileEntry, for_link: ELink | str | None = None
    ) -> ELink:
        new_linkdict = {
            "rel": str(rel),
...
class Schema(KeyFSSchema):
    ...
    INDEX = KeyFSSchema.decl_ptypedkey("INDEX", "{user}/{index}/.config", dict, DictViewReadonly)
    INDEXLIST = KeyFSSchema.decl_ptypedkey("INDEXLIST", "{user}/.indexes", set, SetViewReadonly)

    # type mirror related data
    PYPIFILE_NOMD5 = KeyFSSchema.decl_ptypedkey("PYPIFILE_NOMD5", "{user}/{index}/+e/{dirname}/{basename}", dict, DictViewReadonly)
    MIRRORNAMESINIT = KeyFSSchema.decl_ptypedkey("MIRRORNAMESINIT", "{user}/{index}/.mirrornameschange", int, int)

    # type "stage" related
    PROJSIMPLELINKS = KeyFSSchema.decl_ptypedkey("PROJSIMPLELINKS", "{user}/{index}/{project}/.simple", dict, DictViewReadonly)
    PROJVERSIONS = KeyFSSchema.decl_ptypedkey("PROJVERSIONS", "{user}/{index}/{project}/.versions", set, SetViewReadonly)
    PROJVERSION = KeyFSSchema.decl_ptypedkey("PROJVERSION", "{user}/{index}/{project}/{version}/.config", dict, DictViewReadonly)
    PROJNAMES = KeyFSSchema.decl_ptypedkey("PROJNAMES", "{user}/{index}/.projects", set, SetViewReadonly)
    VERSIONFILELIST = KeyFSSchema.decl_ptypedkey("VERSIONFILELIST", "{user}/{index}/{project}/{version}/.files", set, SetViewReadonly)
    VERSIONFILE = KeyFSSchema.decl_ptypedkey("VERSIONFILE", "{user}/{index}/{project}/{version}/{filename}", dict, DictViewReadonly)
