...
class BaseStage:
    ...
    def __init__(self, xom, username, index, ixconfig, customizer_cls):
        ...
        self.key_project = cast("PatternedKey[dict]", self.keyfs.PROJECT).search(
            user=username, index=index
        )
    ...
    def store_toxresult(
        self, link, content_or_file, *, filename=None, hashes, last_modified=None
    ):
        if self.customizer.readonly:
...
class PrivateStage(BaseStage):
    ...
    def key_doczip(
        ...
        key = cast("PatternedKey[dict]", self.keyfs.DOCZIP)
        (kw, meth) = (
            ({}, key.search) if version is None else (dict(version=version), key.locate)
        )
        return meth(
    ...
    def key_versionfile(
        ...
        key = cast("PatternedKey[dict]", self.keyfs.VERSIONFILE)
        (kw, meth) = (
            ({}, key.search)
            if filename is None
            else (dict(filename=filename), key.locate)
    ...
    def add_project_name(self, project):
        project = normalize_name(project)
        if not self.key_project(project).exists() and self.customizer.readonly:
            raise ReadonlyIndex("index is marked read only")
    ...
    def get_simplelinks_perstage(self, project: NormalizedName | str) -> SimpleLinks:
        ...
        for k, v in self.key_version(project).iter_ulidkey_values():
            versionfile_keys.append(
                self.key_versionfile(project=project, version=k.name)
            )
            versions[k.ulid] = v
        links = self.SimpleLinks([])
        username = self.username
        index = self.index
        for k, v in self.keyfs.tx.iter_ulidkey_values_for(versionfile_keys):
            versions[k.ulid] = v
            ...
            if hash_spec := Digests(v["hashes"]).best_available_spec:
                href += "#" + hash_spec
            links.append(
                SimplelinkMeta((k.name, href, version.get("requires_python"), None))
            )
        return links
    ...
    def has_project_perstage(self, project):
        return self.key_project(project).exists()
    ...
    def store_releasefile(
        self, project, version, filename, content_or_file, *, hashes, last_modified=None
    ):
        if self.customizer.readonly:
            raise ReadonlyIndex("index is marked read only")
        project = normalize_name(project)
    ...
    def store_doczip(
        self, project, version, content_or_file, *, hashes, last_modified=None
    ):
        if self.customizer.readonly:
            raise ReadonlyIndex("index is marked read only")
        project = normalize_name(project)
    ...
    def get_doczip_link(self, project, version):
        ...
        if not doczip:
            return None
        return ELink(
            self.filestore, doczip, self.username, self.index, project, version
        )
...
class MutableLinkStore(LinkStore):
    ...
    def key_doczip(self) -> LocatedKey[dict]:
    ...
    @overload
    def key_versionfile(self, filename: None = None) -> SearchKey[dict]: ...
...
class SimplelinkMeta:
    ...
    def path(self):
        if self.__relpath is notset:
            self.__parse_url()
        return f"{self.user}/{self.index}/{self.__relpath}"
