...
class RootModel:
    ...
    def get_userlist(self):
        ...
        return [
            User(self, key.name)
            for key, v in self.key_user().iter_ulidkey_values()]

    def get_usernames(self):
        return {
            key.params['user']
            for key in self.key_user().iter_ulidkeys()}
...
class BaseStage:
    ...
    def get_keys_for_entrypaths(self, entrypaths):
        ...
        for entrypath in entrypaths:
            ...
            keys.append(
                self.keyfs.match_key(relpath, self.keyfs.PYPIFILE_NOMD5, self.keyfs.STAGEFILE))
        return keys

    def get_entries_for_keys(self, keys):
        key_to_ulidkey = dict(self.keyfs.tx.resolve_keys(keys, fetch=True, fill_cache=True, new_for_missing=False))
        result = []
        for key in keys:
            ulid_key = key_to_ulidkey.get(key)
            result.append(
                None
                if ulid_key is None else
                FileEntry(ulid_key))
        return result
    ...
    def get_last_project_change_serial_perstage(self, project, at_serial=None):
        ...
        if projectname in (absent, deleted):
            ...
            return last_serial
        for version_keydata in tx.conn.iter_keys_at_serial((self.key_projversion(project),), at_serial=at_serial, fill_cache=False, with_deleted=True):
...
class PrivateStage(BaseStage):
    ...
    def del_versiondata(self, project, version, cleanup=True):
        ...
        if cleanup:
            self._regen_simplelinks(project)
            has_versions = next(self.key_projversion(project).iter_ulidkeys(), absent) is not absent
            if not has_versions:
    ...
    def get_last_change_serial_perstage(self, at_serial=None):
        ...
        for project_keydata in tx.conn.iter_keys_at_serial((self.key_projectname,), at_serial=at_serial, fill_cache=False, with_deleted=True):
            ...
            for version_keydata in tx.conn.iter_keys_at_serial((self.key_projversion(project),), at_serial=at_serial, fill_cache=False, with_deleted=True):
                ...
                version = version_keydata.key.name
                try:
                    (versionfiles_serial, versionfilenames) = tx.last_serial_and_value_at(
                ...
                for filename in versionfilenames:
                    try:
                        (versionfile_serial, versionfile_info) = tx.last_serial_and_value_at(
        ...
        (index_serial, index_config) = tx.last_serial_and_value_at(
            index_key, at_serial)
    ...
    def get_links(self, rel=None, basename=None, entrypath=None,
        ...
        def fil(link):
            return (not rel or rel == link.rel) and \
                   ...
                   (not entrypath or entrypath in (link.entrypath, link.relpath)) and \
...
class MutableLinkStore(LinkStore):
    ...
    def remove_links(self, rel=None, basename=None, for_entrypath=None):
        ...
        was_deleted = []
        if del_links:
            with self.stage.key_versionfilelist(self.project, self.version).update() as versionfilenames:
                for link in del_links:
                    ...
                    self.stage.key_versionfile(self.project, self.version, filename).delete()
                    was_deleted.append(link.relpath)
                    threadlog.info("deleted %r link %s", link.rel, link.relpath)
        if was_deleted and self.stage.key_versionfilelist(self.project, self.version).get():
    ...
    def _add_link_to_file_entry(self, rel, file_entry, for_link=None):
        ...
        with self.stage.key_versionfilelist(self.project, self.version).update() as versionfilelist:
...
def register_keys(xom, keyfs):
    ...
    keyfs.register_named_key("PYPIFILE_NOMD5", "+e/{dirname}/{basename}", index_key, dict)
    ...
    keyfs.notifier.on_key_change(keyfs.PROJVERSION.key_name, sub.on_changed_version_config)
    keyfs.notifier.on_key_change(keyfs.STAGEFILE.key_name, sub.on_changed_file_entry)
    keyfs.notifier.on_key_change(keyfs.MIRRORNAMESINIT.key_name, sub.on_mirror_initialnames)
        ...
        with keyfs.read_transaction(at_serial=ev.at_serial) as tx:
            ...
            if metadata != old:
                ...
                hook.devpiserver_on_changed_versiondata(
                    stage=stage,
                    project=source["name"], version=source["version"],
