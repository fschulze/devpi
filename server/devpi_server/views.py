...
@hookimpl(trylast=True)
def devpiserver_on_toxresult_upload_forbidden(request):
    return TOXRESULT_UPLOAD_FORBIDDEN
...
class PyPIView:
    ...
    def post_toxresult(self):
        if not self.request.has_permission("toxresult_upload"):
            msg = self.xom.config.hook.devpiserver_on_toxresult_upload_forbidden(
                request=self.request)
            apireturn(403, msg)
        ...
        tox_result_handling = ToxResultHandling()
        tox_result_handling = self.xom.config.hook.devpiserver_on_toxresult_store(
            request=self.request, tox_result_handling=tox_result_handling)
