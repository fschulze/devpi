...
class PyPIView:
    ...
    def project_modify(self) -> HTTPResponse:
        ...
        if isinstance(json, list):
            try:
                (json, keep_unknown) = stage.get_projectconfig_fields().apply_actions(
                    stage.get_projectconfig_mutable(project), get_actions(json)
                )
            except MissingValueConfigError as e:
                apireturn(400, message=", ".join(e.messages))
        else:
    ...
    def version_yank(self, json: dict) -> Response:
        ...
        links = stage.yank_version(project, version, reason)
        action = "unyanked" if reason is False else "yanked"
        for link in links:
            link.add_log(action, self.request.authenticated_userid)
        return apiresult(HTTPOk, f"version {action}")
    ...
    def _make_view_verdata(self, verdata):
        ...
        if elinks is not None:
            ...
            for linkdict in elinks:
                ...
                if "log" in linkdict:
                    linkdict["log"] = list(linkdict.pop("log"))
    ...
    def releasefile_post(self) -> Response:
        ...
        match json_type := json.get("type"):
            case None:
                if "toxversion" not in json:
                    threadlog.warn(
                        "Got json upload without 'toxversion' or 'type', assuming toxresult."
                    )
                return self.post_toxresult()
            case "yank":
    ...
    def releasefile_yank(self, json: dict) -> Response:
        ...
        link = stage.yank_releasefile(link, reason)
        action = "unyanked" if reason is False else "yanked"
        if link is not None:
            link.add_log(action, self.request.authenticated_userid)
        return apiresult(HTTPOk, f"release {action}")
