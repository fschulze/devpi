...
class PrimaryChangelogRequest:
    ...
    def verify_primary(self) -> None:
        ...
        expected_uuid = self.request.headers.get(H_EXPECTED_PRIMARY_ID)
        primary_uuid = self.xom.config.get_primary_uuid()
        # we require the header but it is allowed to be empty
        # (during initialization)
        if expected_uuid is None:
            msg = f"replica sent no {H_EXPECTED_PRIMARY_ID} header"
            threadlog.error(msg)
...
class ReplicaConnection:
    ...
    log: TagLogger
    non_streaming_content_type = REPLICA_CONTENT_TYPE_V2
    streaming_content_type = REPLICA_CONTENT_TYPE_V2_STREAM
    streaming_content_types = frozenset({REPLICA_CONTENT_TYPE_V2_STREAM})

    def __init__(
        self,
        xom,
        continue_on_missing_remote_primary_uuid,
        set_primary_uuid,
        update_primary_serial,
    ):
    ...
    def fetch(self, handler, url, import_changes):  # noqa: PLR0911,PLR0912
        ...
        with contextlib.ExitStack() as cstack:
            try:
                headers = {
                    "Accept": self.streaming_content_type
                    if self.use_streaming
                    else self.non_streaming_content_type
                }
                r = self.http.stream(
                    cstack,
                    "GET",
            ...
            if (
                not remote_primary_uuid
                and not self.continue_on_missing_remote_primary_uuid(r.headers)
            ):
                return (True, None)
            if primary_uuid and remote_primary_uuid != primary_uuid:
                ...
                log.error(
                    "FATAL: primary UUID %r does not match "
    ...
    def handler_multi(self, response, import_changes):
        ...
        use_stream = content_type in self.streaming_content_types
        method = load_iter if content_type == REPLICA_CONTENT_TYPE_V2 else load
        if use_stream:
            with contextlib.closing(response):
                ...
                try:
                    while True:
                        serial = load(stream)
                        changes = method(stream)
                        import_changes(serial, changes)
                        self.update_primary_serial(
                            serial, update_sync=False, ignore_lower=True
                        )
            ...
            readableiterable = ReadableIterator(response.iter_bytes(65536))
...
class ReplicaThread:
    ...
    def continue_on_missing_remote_primary_uuid(self, headers):
        ...
        self.log.error(
            "remote provides no %r header, headers were: %s", H_PRIMARY_UUID, headers
        )
    ...
    def get_changes(self, serial: int, changes: Iterable[tuple]) -> Sequence[KeyData]:
        ...
        for keyname, relpath, ulid, parent_ulid, back_serial, val in changes:
            ...
            ulidkey = key.make_ulid_key(
                ULID(ulid),
                parent_ulid=None if parent_ulid is None else ULID(parent_ulid),
            )
            result.append(
                KeyData(
                    key=ulidkey,
    ...
    def tick(self) -> None:
        ...
        (result, _exc) = self.fetch_multi(serial)
        if not result:
...
def register_key_subscribers(xom: XOM) -> None:
    xom.keyfs.notifier.on_key_change(
        xom.keyfs.schema.PROJECT, ProjectChanged(xom).handler
    )
...
class FileReplicationSharedData:
    ...
    def __init__(self, xom: XOM) -> None:
        ...
        # set whenever the download and error queue is empty and the initial
        # queue thread has finished
    ...
    def process_next_errored(self, handler):
        try:
            ...
            if self.queue.empty() and self.init_queue_finished_at is not None:
        ...
        with keyfs.read_transaction(at_serial=serial):
            ...
            if link is not None:
                self.xom.config.hook.devpiserver_on_replicated_file(
                    ...
                    serial=serial,
                    back_serial=back_serial,
                    is_from_mirror=index_type == IndexType("remote"),
...
class ProjectChanged:
    ...
    def handler(self, ev: KeyChangeEvent) -> None:
        threadlog.debug("ProjectChanged %s", ev.data.key)
        cache = ev.data.value
