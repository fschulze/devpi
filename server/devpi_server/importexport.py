...
class Migrator:
    ...
    def migrate_remote_index(self, indexconfig: dict) -> dict:
        ...
        if "mirror_use_external_urls" in indexconfig:
            indexconfig["remote_use_external_urls"] = indexconfig.pop(
                "mirror_use_external_urls"
            )
        if "mirror_web_url_fmt" in indexconfig:
            indexconfig["remote_web_url_fmt"] = indexconfig.pop("mirror_web_url_fmt")
        return indexconfig
...
class Importer:
    ...
    def import_all(self, path: Path) -> None:  # noqa: PLR0912
        ...
        with self.xom.keyfs.write_transaction():
            for username, userconfig in self.import_users.items():
                ...
                user._modify(**{k: v for k, v in userconfig.items() if k != "username"})
    ...
    def import_filedesc(self, stage: BaseIndex, filedesc: dict, versions: dict) -> None:  # noqa: PLR0912
        ...
        if filedesc["type"] == Rel.ReleaseFile:
            ...
            f.close()
            raise Fatal(msg)
        if (error := entry.validate(f)) is not None:
            msg = f"{p}: {error}"
            f.close()
            raise Fatal(msg)
        if link is not None:
