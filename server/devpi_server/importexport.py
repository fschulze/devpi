...
class IndexDump:
    ...
    def dump_releasefiles(self, linkstore):
        for link in linkstore.get_links(rel=Rel.ReleaseFile):
            ...
            relpath = self.exporter.copy_file(
                entry,
                self.basedir / linkstore.project / link.version / entry.basename)
            self.add_filedesc(
                Rel.ReleaseFile, linkstore.project, relpath,
...
class Importer:
    ...
    def wait_for_events(self):
        ...
        while True:
            ...
            self.tw.line(
                f"waiting for events until {wait_serial}/{latest_serial}")
            keyfs.notifier.wait_event_serial(wait_serial)
        self.tw.line(
            f"wait_for_events: importing finished; {latest_serial=}")
    ...
    def import_filedesc(self, stage, filedesc, versions):
        ...
        if filedesc["type"] == Rel.ReleaseFile:
            ...
            if hasattr(stage, 'store_releasefile'):
                ...
                if (links_with_data := mirrorlinks.get_links()) is None:
                    links_with_data = []
                ...
                mirrorlinks._save_cache_links(
                    links, requires_python, yanked, None)
