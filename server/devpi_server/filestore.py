...
class Digests(dict):
    ...
    def errors_for(self, content_or_hashes):
        ...
        if isinstance(content_or_hashes, Digests):
            ...
            hashes = get_hashes(content_or_hashes, self.keys())
        for hash_type, hash_value in hashes.items():
            if (expected_hash_value := self[hash_type]) is None:
                continue
...
def get_default_hash_spec(content_or_file):
    return get_hashes(content_or_file, (DEFAULT_HASH_TYPE,)).get_default_spec()
...
def get_default_hash_value(content_or_file):
    return get_hashes(content_or_file, (DEFAULT_HASH_TYPE,)).get_default_value()


def get_file_hash(fp, hash_type):
    return get_hashes(fp, (hash_type,))[hash_type]


def get_hashes(content_or_file, hash_types=DEFAULT_HASH_TYPES):
    if not hash_types:
        return {}
...
def get_hash_value(content_or_file, hash_type):
    return get_hashes(content_or_file, (hash_type,))[hash_type]


def get_hash_spec(content_or_file, hash_type):
    return get_hashes(content_or_file, (hash_type,)).get_spec(hash_type)
