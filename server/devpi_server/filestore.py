...
class FileStore:
    ...
    def iter_maplinks(self, links: Sequence[URL], user: str, index: str, project: str) -> Iterator[MutableFileEntry]:
        if not links:
            return
        keyfs = self.keyfs
        index_key = self.keyfs.INDEX(user=user, index=index).resolve(fetch=True)
        key_links = {
            key_from_link(keyfs, link, index_key): link
            for link in links}
        # pre-create/fetch all keys
        key_entries = {
            key: MutableFileEntry(ulid_key)
            for (key, ulid_key) in keyfs.tx.resolve_keys(key_links, fetch=True, fill_cache=True, new_for_missing=True)}
    ...
    def maplink(self, link: URL, user: str, index: str, project: str) -> MutableFileEntry:
        (entry,) = self.iter_maplinks((link,), user, index, project)
        return entry

    def get_file_entry(self, relpath):
        if (key := self.keyfs.match_key(relpath, self.keyfs.FILE_NOHASH, self.keyfs.FILE)):
