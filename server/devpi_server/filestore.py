...
class FileStore:
    ...
    def store(
        self, user, index, basename, content_or_file, *, ref_hash_spec=None, hashes
    ):
...
class BaseFileEntry:
    ...
    def file_path_info(self):
        return FilePathInfo(
            f"+files/{self.relpath}", self.hashes.get_default_value(None)
        )
    ...
    def file_os_path(self, *, _raises=True):
        ...
        if _raises and self.tx.io_file.is_path_dirty(path):
            raise RuntimeError(
                "Can't access file %s directly during transaction" % path
            )
        return path
