...
class Connection:
    ...
    def _db_write_typedkeys(self, new_typedkeys, updated_typedkeys):
        if new_typedkeys:
            self._sqlaconn.execute(sa.insert(relpath_ulid_table), new_typedkeys)
    ...
    def get_rel_renames(self, serial) -> list | None:
        data = self._sqlaconn.execute(
            sa.select(renames_table.c.data).where(renames_table.c.serial == serial)
        ).scalar()
    ...
    def _write_records(self, serial, records, renames):
        ...
        if ulid_changelog:
            execute(sa.insert(ulid_changelog_table).values(ulid_changelog))
        execute(sa.insert(renames_table).values((serial, dumps(renames))))
