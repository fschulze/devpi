...
if TYPE_CHECKING:
    ...
    KeyFSTypesRO = Union[bool, bytes, DictViewReadonly[str, Readonly], float, frozenset, int, ListViewReadonly, SetViewReadonly, str, TupleViewReadonly]
    KeyFSTypes = Union[bool, bytes, dict[str, object], float, frozenset, int, list, set, str, tuple]
...
class ULID(int):
    ...
    @classmethod
    def new(cls, *, _randbits: Callable = random.getrandbits, _time_ns: Callable = time.time_ns) -> Self:
        ns = _time_ns()
...
def iter_lineage(key: LocatedKey | PatternedKey | ULIDKey) -> Iterator[LocatedKey | PatternedKey | ULIDKey]:
...
class LocatedKey:
    __slots__ = ('_hash', '_keyfs', 'key_name', 'key_type', 'name', 'params', 'parent_key')
    _hash: int
    ...
    def __init__(self, keyfs: KeyFS, key_name: str, name: str, parent_key: LocatedKey | ULIDKey | None, key_type: type[KeyFSTypes], params: dict | None = None) -> None:
    ...
    def make_ulid_key(self, ulid: ULID, *, parent_key: ULIDKey | Absent | None = absent, parent_ulid: ULID | Absent | None = absent) -> ULIDKey:
        ...
        ulid_key = ULIDKey(self.keyfs, self.key_name, self.location, self.name, ulid, parent_ulid, self.key_type, params=self.params)
...
class PatternedKey:
    __slots__ = ('_full_rex_reverse', '_hash', '_keyfs', '_keys', '_rex_reverse', '_simple_pattern_key', 'key_name', 'key_type', 'parent_key', 'pattern')
    ...
    def __init__(self, keyfs: KeyFS, key_name: str, pattern: str, parent_key: PatternedKey | None, key_type: type[KeyFSTypes]) -> None:
        self._keyfs = weakref.ref(keyfs)
        self.key_name = key_name
    ...
    def __call__(self, *args: str, **kw: LocatedKey | ULIDKey | str) -> LocatedKey | SearchKey:
        ...
        parent_key: LocatedKey | SearchKey | ULIDKey | None = None if self.parent_key is None else self.parent_key(**kw)
        ...
        if self.keys.difference(kw):
            return SearchKey(keyfs, self.key_name, self.pattern, parent_key, self.key_type, params=kw)
        if self.simple_pattern_key is not None:
            name = kw[self.simple_pattern_key]
            assert isinstance(name, str)
        else:
            name = self.pattern.format_map(kw)
        return LocatedKey(keyfs, self.key_name, name, parent_key, self.key_type, params=kw)
    ...
    def __eq__(self, other: object) -> bool:
        if not isinstance(other, PatternedKey):
            return NotImplemented
        return self.key_name == other.key_name and self.full_pattern == other.full_pattern
    ...
    def make_ulid_key(self, location: str, name: str, ulid: ULID, parent_key: ULIDKey | Absent | None = absent, parent_ulid: ULID | Absent | None = absent, parent_params: dict | None = None) -> ULIDKey:
        if not isinstance(parent_key, Absent):
            assert isinstance(parent_ulid, Absent)
            parent_ulid = None if parent_key is None else parent_key.ulid
        else:
        ...
        keyfs = self._keyfs()
        assert keyfs is not None
        ulid_key = ULIDKey(keyfs, self.key_name, location, name, ulid, parent_ulid, self.key_type, params=params)
        if isinstance(parent_key, ULIDKey):
            ulid_key._parent_key = parent_key
        return ulid_key
    ...
    def full_rex_reverse(self) -> Pattern:
        ...
        if _full_rex_reverse is None:
            _full_rex_reverse = self._full_rex_reverse = get_rex_reverse(self.full_pattern)
    ...
    def simple_pattern_key(self) -> str | None:
        ...
        if isinstance(_simple_pattern_key, Absent):
            _simple_pattern_key = self._simple_pattern_key = get_simple_pattern_key(self.pattern)
        return _simple_pattern_key


class SearchKey:
    __slots__ = ('_hash', '_keyfs', '_keys', '_parent_ulidkey', '_simple_pattern_key', 'key_name', 'key_type', 'params', 'parent_key', 'pattern')
    _hash: int
    _keyfs: weakref.ReferenceType[KeyFS]
    _keys: frozenset[str]
    ...
    def __init__(self, keyfs: KeyFS, key_name: str, pattern: str, parent_key: LocatedKey | ULIDKey | None, key_type: type[KeyFSTypes], params: dict) -> None:
        self._keyfs = weakref.ref(keyfs)
        self.key_name = key_name
        self.pattern = pattern
        self.parent_key = parent_key
    ...
    def __call__(self, *args: str, **kw: LocatedKey | ULIDKey | str) -> LocatedKey:
        ...
        keyfs = self._keyfs()
        assert keyfs is not None
        return LocatedKey(keyfs, self.key_name, name, self.parent_ulidkey, self.key_type, params=self.params | kw)
    ...
    def iter_ulidkey_values(self, *, fill_cache: bool = True) -> Iterator[tuple[ULIDKey, KeyFSTypesRO]]:
        keyfs = self._keyfs()
        assert keyfs is not None
    ...
    def parent_ulidkey(self) -> ULIDKey | None:
        ...
        if isinstance(_parent_ulidkey, Absent):
            ...
            if isinstance(parent_key, ULIDKey):
                ...
                _parent_ulidkey = self._parent_ulidkey = None if parent_key is None else parent_key.resolve(fetch=False)
...
class ULIDKey:
    __slots__ = ('_parent_key', 'key_name', 'key_type', 'keyfs', 'location', 'name', 'params', 'parent_ulid', 'ulid')
    _parent_key: ULIDKey
    keyfs: KeyFS
    key_name: str
    key_type: type[KeyFSTypes]
    ...
    def __init__(self, keyfs: KeyFS, key_name: str, location: str, name: str, ulid: ULID, parent_ulid: ULID | None, key_type: type[KeyFSTypes], params: dict | None = None) -> None:
        self.keyfs = keyfs
        self.key_name = key_name
    ...
    def new_ulid(self) -> ULIDKey:
        return ULIDKey(self.keyfs, self.key_name, self.location, self.name, ULID.new(), self.parent_ulid, self.key_type, params=self.params)
    ...
    def parent_key(self) -> ULIDKey | None:
        ...
        if _parent_key is None:
            _parent_key = self._parent_key = self.keyfs.tx.key_for_ulid(self.parent_ulid)
        return _parent_key
