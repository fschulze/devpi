...
class Record(Generic[KeyType, KeyTypeRO]):
    key: ULIDKey[KeyType, KeyTypeRO]
    value: KeyType | Deleted
    back_serial: int
    old_key: ULIDKey[KeyType, KeyTypeRO] | Absent
    old_value: KeyTypeRO | Absent | Deleted
...
@frozen
class KeyData(Generic[KeyType, KeyTypeRO]):
    key: ULIDKey[KeyType, KeyTypeRO]
    serial: int
    back_serial: int
    value: KeyTypeRO | Deleted
...
class LocatedKey(Generic[KeyType, KeyTypeRO]):
    ...
    def resolve(self, *, fetch: bool) -> ULIDKey[KeyType, KeyTypeRO]:
    ...
    def with_resolved_parent(self) -> LocatedKey[KeyType, KeyTypeRO]:
        parent_key = self.parent_key
        if parent_key is None or isinstance(parent_key, ULIDKey):
            return self
        keyfs = self._keyfs()
        assert keyfs is not None
        parent_ulidkey = keyfs.tx.resolve(parent_key, fetch=False)
        return LocatedKey(
            keyfs,
            self.key_name,
            self.name,
...
class PatternedKey(Generic[KeyType, KeyTypeRO]):
    ...
    ) -> LocatedKey[KeyType, KeyTypeRO] | SearchKey[KeyType, KeyTypeRO]:
...
class SearchKey(Generic[KeyType, KeyTypeRO]):
    __slots__ = (
        "_hash",
        "_keyfs",
        "_keys",
    ...
    ) -> Iterator[tuple[ULIDKey[KeyType, KeyTypeRO], KeyTypeRO]]:
        assert self.has_resolved_parent()
        keyfs = self._keyfs()
        assert keyfs is not None
        yield from cast(
            "Iterator[tuple[ULIDKey[KeyType, KeyTypeRO], KeyTypeRO]]",
            keyfs.tx.iter_ulidkey_values_for((self,), fill_cache=fill_cache),
        )
    ...
    def with_resolved_parent(self) -> SearchKey[KeyType, KeyTypeRO]:
        parent_key = self.parent_key
        if parent_key is None or isinstance(parent_key, ULIDKey):
            return self
        keyfs = self._keyfs()
