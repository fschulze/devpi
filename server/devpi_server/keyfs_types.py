...
class LocatedKey:
    ...
    def __init__(self, keyfs: KeyFS, key_name: str, location: str, key_type: type[KeyFSTypes], params: dict | None = None) -> None:
    ...
    def make_ulid_key(self, ulid: ULID) -> ULIDKey:
        return ULIDKey(self.keyfs, self.key_name, self.location, ulid, self.key_type, params=self.params)
...
class PatternedKey:
    __slots__ = ('_keyfs', '_rex_reverse', 'key_name', 'key_type', 'parent_key', 'pattern')
    _keyfs: weakref.ReferenceType[KeyFS]
...
class ULIDKey:
    __slots__ = ('keyfs', 'key_name', 'key_type', 'location', 'params', 'ulid')
    keyfs: KeyFS
    key_name: str
    key_type: type[KeyFSTypes]
    location: str
    ...
    def __init__(self, keyfs: KeyFS, key_name: str, location: str, ulid: ULID, key_type: type[KeyFSTypes], params: dict | None = None) -> None:
        self.keyfs = keyfs
        self.key_name = key_name
        assert '{' not in location
        self.location = location
    ...
    def new_ulid(self) -> ULIDKey:
        return ULIDKey(self.keyfs, self.key_name, self.location, ULID.new(), self.key_type, params=self.params)
