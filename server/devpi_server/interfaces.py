...
class IDBIOFileConnection(Interface):
    ...
    def io_file_set(path: FilePathInfo, content_or_file: Union[bytes, IO[bytes]]) -> None:
...
class IIOFile(Interface):
    ...
    def set_content(path: FilePathInfo, content_or_file: Union[bytes, IO[bytes]]) -> None:
        """ Set the binary content of the file at path. """
...
class IWriter(Interface):
    ...
    def __exit__(cls: Optional[type[BaseException]], val: Optional[BaseException], tb: Optional[TracebackType]) -> None:  # noqa: PLE0302 PYI036
        pass

    def record_set(typedkey: Union[PTypedKey, TypedKey], value: Any, back_serial: int) -> None:
        pass


class IWriter2(IWriter):
    commit_serial = Attribute("""
        The current to be commited serial set when entering the context manager. """)
...
class IOFileConnectionAdapter:
    def __init__(self, conn: Any) -> None:
        ...
        self.commit_files_without_increasing_serial = conn.commit_files_without_increasing_serial
    ...
    def io_file_set(self, path: FilePathInfo, content_or_file: Union[bytes, IO[bytes]]) -> None:
