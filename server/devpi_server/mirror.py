...
class MirrorStage(BaseStage):
    ...
    def _save_cache_links(self, project, links, requires_python, yanked, serial, etag):
        ...
        if old != data:
            threadlog.debug("processing changed simplelinks for %s in %s", project, self.index)
            num_unchanged = 0
            num_changed = 0
            num_deleted = 0
            num_new = 0
            ...
            for _k, ulid_key in self.keyfs.tx.resolve_keys(new_keys, fetch=True, fill_cache=True, new_for_missing=True):
                num_new += 1
                ulid_key.set(data[ulid_key.name])
            threadlog.debug("%s new, %s deleted, %s changed, and %s unchanged simplelinks for %s in %s", num_new, num_deleted, num_changed, num_unchanged, project, self.index)
    ...
    async def _async_fetch_releaselinks(self, newlinks_future, project, cache_serial, etag, _key_from_link):
