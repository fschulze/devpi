...
def iter_cache_remote_file(stage, entry, url):
    ...
    with ExitStack() as cstack:
        ...
        except Exception as err:  # noqa: BLE001 - log all exceptions
...
def iter_remote_file_replica(stage, entry, url):
    ...
    with ExitStack() as cstack:
        r = stage.http.stream(
            cstack, "GET", primary_url, allow_redirects=True,
...
class HTTPClient:
    ...
    def stream(self, cstack, method, url, *, allow_redirects, timeout=None, extra_headers=None):
        ...
        if response.status_code in (401, 403) and self.update_auth_candidates(
            ...
            return self.stream(
                cstack, method, url, allow_redirects=allow_redirects,
                timeout=timeout, extra_headers=extra_headers)
        return response
...
class MirrorLinks:
    ...
    def are_expired(self):
        if self.key_projectname.exists():
            return self.stage.cache_retrieve_times.is_expired(self.project, self.stage.cache_expiry)
        return True
    ...
    def delete_version(self, version):
        ...
        if (links := self.get_links()) is not None:
            ...
            entries = [
                x for x in entries
                if x.version == version and x.file_exists()]
            for entry in entries:
    ...
    def get_fresh_links(self):
        ...
        if self.stage.offline:
            entries = self.stage.get_entries_for_entrypaths(x[1] for x in links_with_data)
    ...
    def _save_cache_links(self, links, requires_python, yanked, cache_info):
        ...
        if old != data:
            ...
            for _k, ulid_key in tx.resolve_keys(new_keys, fetch=True, fill_cache=True, new_for_missing=True):
                num_new += 1
                ulid_key.set(data[ulid_key.name])
            threadlog.debug(
                "%s new, %s deleted, %s changed, and %s unchanged simplelinks for %s in %s",
                num_new, num_deleted, num_changed, num_unchanged, self.project, self.stage.index)
        ...
        def on_commit():
            ...
            self.stage.cache_retrieve_times.refresh(self.project, cache_info.get("etag"))
...
class MirrorStage(BaseStage):
    ...
    def __init__(self, xom, username, index, ixconfig, customizer_cls):
        ...
        self.projects_timeout = max(
            self.timeout,
            60 if self.xom.is_replica() else 30)
    ...
    def http(self):
        if self.xom.is_replica():
            ...
            get_extra_headers = self.xom.replica_thread.connection.http.get_extra_headers
        else:
    ...
    async def _get_remote_projects(self, projects_future: asyncio.Future) -> None:
        ...
        if response.status_code == 304:
            raise self.UpstreamNotModified(
                f"{response.status_code} status on GET {self.mirror_url!r}",
                etag=etag)
    ...
    def _update_projects(self):
        ...
        try:
            ...
            threadlog.warn(
                ...
                self.index, self.projects_timeout)
    ...
    async def _async_fetch_releaselinks(self, newlinks_future, project, cache_info, _key_from_link):
    ...
    def _update_simplelinks(self, project, info, links, newlinks):
        ...
        with self.keyfs.write_transaction(allow_restart=True):
            ...
            linkstomap = [
                newlink for newinfo, link in zip(newlinks, info["releaselinks"])
                if (newlink := link) not in existing_info]
            # looping over iter_maplinks creates the entries in the database
            for _entry in self.filestore.iter_maplinks(linkstomap, self.user.name, self.index, project):
                pass
    ...
    def get_simplelinks_perstage(self, project):
        ...
        _key_from_link = partial(
            key_from_link, self.keyfs, index_key=index_key)
        try:
            self.xom.run_coroutine_threadsafe(
                self._async_fetch_releaselinks(
                    newlinks_future, project, mirrorlinks.get_cache_info(), _key_from_link),
    ...
    def get_last_project_change_serial_perstage(self, project, at_serial=None):
        ...
        if projectname in (absent, deleted):
            ...
            return last_serial
        for mirrorfile_key in tx.conn.iter_ulidkeys_at_serial((self.key_mirrorfile(project),), at_serial=at_serial, fill_cache=False, with_deleted=True):
