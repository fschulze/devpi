...
class BaseIndex:
    ...
    def get_indexconfig_from_kwargs(
        ...
        fields = self.get_indexconfig_fields()
        fields.fill_config_from_kwargs(ixconfig, kwargs)
        ixconfig["type"] = self.index_type
    ...
    def get_indexconfig_fields(self) -> ConfigFields:
        fields = ConfigFields([])
        index_type = self.index_type
        fields.extend(
            self._get_indexconfig_fields(),
            f"The {index_type!r} index defines fields which "
            "conflict with standard index configuration fields: {conflicting}",
        )
        fields.extend(
    ...
    def get_projectconfig_fields(self) -> ConfigFields:
        fields = ConfigFields([])
        index_type = self.index_type
        fields.extend(
            self._get_projectconfig_fields(),
            f"The {index_type!r} index defines fields which "
            "conflict with standard project configuration fields: {conflicting}",
        )
        fields.extend(
    ...
    def get_projectconfig_from_kwargs(
        ...
        config: dict[str, Any] = {}
        fields = self.get_projectconfig_fields()
