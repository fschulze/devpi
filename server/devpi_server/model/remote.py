...
REMOTEFILE_IGNORED_KEYS = frozenset({"hashes", "size"})
...
class RemoteData:
    ...
    def save_links(  # noqa: PLR0912
        ...
        def on_commit():
            ...
            stage.cache_retrieve_times.refresh(project, cache_info["etag"])
...
class RemoteIndex(BaseIndex):
    ...
    def key_remoteprojectinfo(
        ...
        key = self.keyfs.schema.REMOTEPROJECTINFO
        (kw, meth) = (
            ({}, key.search)
            if project is None
    ...
    def _get_indexconfig_fields(self) -> Sequence[ConfigField]:
        return [
            ...
            ConfigField(
                name="remote_ignore_serial_header", normalize=ensure_boolean, type=bool
            ),
            ConfigField(
            ...
            ConfigField(
                name="remote_url", normalize=self.normalize_remote_url, type=str
            ),
            ConfigField(
                name="remote_use_external_urls", normalize=ensure_boolean, type=bool
            ),
            ConfigField(name="remote_web_url_fmt", normalize=str, type=str),
    ...
    def _get_projectconfig_fields(self) -> Sequence[ConfigField]:
        return []
    ...
    def del_project(self, project: NormalizedName | str) -> None:
        ...
        self.key_remoteprojectinfo(project).with_resolved_parent().delete()
    ...
    def clear_simplelinks_cache(self, project: NormalizedName | str) -> None:
        ...
        self.cache_retrieve_times.expire(project)
        self.key_remoteprojectinfo(project).with_resolved_parent().set({})
    ...
    def get_simplelinks_perstage(self, project: NormalizedName | str) -> SimpleLinks:  # noqa: PLR0911, PLR0912
        ...
        if lock is not None:
            self.keyfs.tx.on_finished(lock.release)
        remotedata = self._get_remotedata(project)
        is_expired = remotedata.is_expired()
