    ...
    def _get_relpaths_at(self, relpaths, serial):
        ...
        relpaths_info_stmt = (
            sa.select(
                ...
                sa.func.max(ulid_changelog_table.c.serial).label("max_serial"),
                sa.func.coalesce(
            ...
            .join(
                ulid_changelog_table,
                relpaths_sq.c.ulid == ulid_changelog_table.c.ulid)
            .where(
                ulid_changelog_table.c.serial <= serial))
        ...
        for relpath, ulid, keytype, max_serial, deleted_serial in execute(relpaths_info_stmt):
            if (keytype_cls := typed_keys[keytype].type) in (dict, set):
                ulid_obj_map[ulid] = None if deleted_serial >= max_serial else keytype_cls()
        ...
        latest_key_stmt = (
            sa.select(
                ...
                ulid_changelog_sq.c.key)
            .group_by(ulid_changelog_sq.c.ulid, ulid_changelog_sq.c.key))
        ...
        stmt = (
            ...
            .join(
                latest_key_sq,
                sa.and_(
                    latest_key_sq.c.ulid == ulid_changelog_sq.c.ulid,
                    latest_key_sq.c.serial == ulid_changelog_sq.c.serial,
                    latest_key_sq.c.key == ulid_changelog_sq.c.key))
            .where(ulid_changelog_sq.c.value.isnot(None))
        ...
        for ulid, obj in ulid_obj_map.items():
        ...
        for ulid, key, value in rows:
            if current_ulid != ulid:
                current_ulid = ulid
                if ulid in ulid_obj_map:
                    current_obj = ulid_obj_map[ulid]
                else:
                    result[ulid_relpath_map[ulid]] = dict(
                        last_serial=ulid_serial_map[ulid],
                        back_serial=ulid_back_serial_map[ulid],
