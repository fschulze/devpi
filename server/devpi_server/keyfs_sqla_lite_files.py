...
class Storage(BaseStorage):
    ...
    def __init__(self, basedir: Path, *, notify_on_commit: Callable, settings: dict) -> None:
        ...
        self.rw_engine = sa.create_engine(self._url(mode="rw"), echo=False, poolclass=sa.NullPool)
    ...
    def ensure_tables_exist(self) -> None:
        ...
        if not self.sqlpath.exists():
            engine = sa.create_engine(self._url(mode="rwc"), echo=False, poolclass=sa.NullPool)
    ...
    def get_connection(self, *, closing: bool = True, write: bool = False, timeout: int = 30) -> Connection | AbstractContextManager[Connection]:
...
def devpiserver_describe_storage_backend(settings: dict) -> StorageInfo:
    return StorageInfo(
        ...
        description="sqlite backend using SQLAlchemy",
