...
def test_version_view(mapp, testapp, monkeypatch):
    ...
    api = mapp.create_and_use()
    mapp.upload_file_pypi(
        "pkg1-2.6.tar.gz", b"contentveryold", "pkg1", "2.6").file_url_no_hash
    mapp.upload_file_pypi(
        "pkg1-2.6.tar.gz", b"contentold", "pkg1", "2.6").file_url_no_hash
    ...
    assert [x[:2] for x in filesinfo] == [
        (['pkg1-2.6.tar.gz', 'Size', '7', 'bytes', 'Type', 'Source', 'sha256', 'ed7002b439e9ac845f22357d822bac1444730fbdb6016d3ec9432297b9ec9f73'], []),
        (['pkg1-2.6.zip', 'Size', '10', 'bytes', 'Type', 'Source', 'sha256', '43791c10b26ca70c9641ef652d2e77c6c77198f25c6a3f736799cfc5c1e77d02'], [])
    ]
...
def test_complex_name(mapp, testapp):
    ...
    assert [(compareable_text(l.text), l.attrs['href']) for l in links] == [
        ...
        (
            ...
            'http://localhost/user1/dev/+f/ed7/002b439e9ac84/%s-0.9.tar.gz' % pkgname)]
    r = testapp.xget(
