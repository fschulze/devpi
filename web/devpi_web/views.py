...
def index_get(context, request):
    ...
    index_title = stage.ixconfig.get('title', None)
    if index_title:
        title = f"{stage.name} ({index_title}) index"
    else:
        title = f"{stage.name} index"
    result = dict(
        _context=context,
        title=title,
        nav_links=[dict(
    ...
    for project in sorted(stage.list_projects_perstage()):
        ...
        def add_version(version, is_pre_release):
            nonlocal versions
        ...
        if stable_version is not None:
            add_version(stable_version, is_pre_release=False)
        if latest_version is not None and stable_version != latest_version:
            add_version(latest_version, is_pre_release=True)
