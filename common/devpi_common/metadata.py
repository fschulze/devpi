...
def version_sort_string(version):
    ...
    result = (
        ...
        encode_iterable(version.pre) if version.pre else "~",
        encode_iterable(version.post) if version.post else "!",
        encode_iterable(version.dev) if version.dev else "~",
        encode_local(version.local) if version.local else "!")
    return "".join(result)
