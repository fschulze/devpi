...
def wait_for_port(host: str, port: int, timeout: float = 60) -> None:
    while timeout > 0:
        ...
        timeout -= 1
    raise RuntimeError(
        f"The port {port} on host {host} didn't become accessible")
...
def devpipostgresql_postgresql(request: pytest.FixtureRequest) -> Generator[dict, None, None]:
    tmpdir = Path(
...
class Storage(main.Storage):
    ...
    @overload
    def get_connection(self, *, closing: Literal[True], write: bool = False, timeout: int = 30) -> AbstractContextManager[main.Connection]:
        pass

    @overload
    def get_connection(self, *, closing: Literal[False], write: bool = False, timeout: int = 30) -> main.Connection:
        pass

    def get_connection(self, *, closing: bool = True, write: bool = False, timeout: int = 30) -> main.Connection | AbstractContextManager[main.Connection]:
        conn = main.Storage.get_connection(self, closing=False, write=write, timeout=timeout)
        self._connections.append((conn, closing, conn.storage.database, time.monotonic()))
        if closing:
...
def _devpipostgresql_devpiserver_describe_storage_backend_mock(request: pytest.FixtureRequest) -> None:
