...
class Connection(BaseConnection):
    ...
    def _lock(self) -> None:
        self._sqlaconn.execute(sa.select(
            sa.func.pg_advisory_xact_lock(1))).scalar()

    def get_next_serial(self) -> int:
        result = self._sqlaconn.execute(sa.select(
            sa.func.nextval('changelog_serial_seq'))).scalar()
    ...
    def io_file_exists(self, path: FilePathInfo) -> bool:
        ...
        result = self._sqlaconn.execute(
            sa.select(self.files_table.c.path)
            .where(self.files_table.c.path == path.relpath)).scalar()
    ...
    def _select_io_file_get(self, path: FilePathInfo) -> bytes:
        ...
        content = self._sqlaconn.execute(
            sa.select(self.files_table.c.data)
            .where(self.files_table.c.path == path.relpath)).scalar()
        if content is None:
            raise OSError
        return content
    ...
    def _copy_io_file_open(self, path: FilePathInfo) -> IO[bytes]:
        ...
        q = f"""
            ...
            TO STDOUT WITH (FORMAT binary);"""  # noqa: S608 - we escape the input with literal
        stream = FileIn(f)
    ...
    def io_file_set(self, path: FilePathInfo, content_or_file: bytes | IO[bytes]) -> None:
        assert not os.path.isabs(path.relpath)
    ...
    def io_file_size(self, path: FilePathInfo) -> int | None:
        ...
        if not isinstance(f, Absent):
            ...
            return size
        result = self._sqlaconn.execute(
            sa.select(self.files_table.c.size)
            .where(self.files_table.c.path == path.relpath)).scalar()
    ...
    def _insert_file_write(self, path: str, f: IO[bytes]) -> None:
        ...
        self._sqlaconn.execute(
            sa.insert(self.files_table)
            .values(path=path, size=len(content), data=content))

    def _file_delete(self, path: str) -> None:
        assert not os.path.isabs(path)
        assert not path.endswith("-tmp")
        self._sqlaconn.execute(
            sa.delete(self.files_table)
            .where(self.files_table.c.path == path))
...
class Storage(BaseStorage):
    ...
    def __init__(self, basedir: Path, *, notify_on_commit: Callable, settings: dict) -> None:
        ...
        self.use_copy = ensure_boolean(settings.get("use_copy", os.environ.get(
    ...
    @overload
    def get_connection(self, *, closing: Literal[True], write: bool = False, timeout: int = 30) -> AbstractContextManager[Connection]:
        pass

    @overload
    def get_connection(self, *, closing: Literal[False], write: bool = False, timeout: int = 30) -> Connection:
        pass

    def get_connection(self, *, closing: bool = True, write: bool = False, timeout: int = 30) -> Connection | AbstractContextManager[Connection]:  # noqa: ARG002
...
def devpiserver_describe_storage_backend(settings: dict) -> StorageInfo:
    return StorageInfo(
        name="pg8000",
        description="Postgresql backend with support for files in DB",
