...
class Connection:
    ...
    def _iter_serial_and_value_backwards(self, key: LocatedKey, last_serial) -> Iterator[KeyData]:
        while last_serial >= 0:
            ...
            if val is None:
                val = deleted
            yield KeyData(relpath=key.relpath, keyname=keyname, serial=last_serial, back_serial=back_serial, value=val)
    ...
    def get_key_at_serial(self, key: LocatedKey, serial) -> KeyData:
        ...
        if result is absent:
            ...
            if changes is not absent and cache_key in changes:
                (keyname, back_serial, value) = changes[cache_key]
                if value is None:
                    value = deleted
                result = KeyData(relpath=key.relpath, keyname=keyname, serial=serial, back_serial=back_serial, value=value)
        if result is absent:
        ...
        if not isinstance(content_or_file, bytes) and not callable(getattr(content_or_file, "seekable", None)):
            ...
            if len(content_or_file) > 1048576:
                threadlog.warn(
                    ...
                    len(content_or_file) / 1048576, path.relpath)
