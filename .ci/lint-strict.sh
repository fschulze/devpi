...
RUFF_TARGET_VERSION_CLIENT=$(awk '/(target-version).*=.*(.*)$/ { print $1 "=" $3 }' client/pyproject.toml)
RUFF_TARGET_VERSION_COMMON=$(awk '/(target-version).*=.*(.*)$/ { print $1 "=" $3 }' common/pyproject.toml)
RUFF_TARGET_VERSION_DEBUGGING=$(awk '/(target-version).*=.*(.*)$/ { print $1 "=" $3 }' debugging/pyproject.toml)
RUFF_TARGET_VERSION_POSTGRESQL=$(awk '/(target-version).*=.*(.*)$/ { print $1 "=" $3 }' postgresql/pyproject.toml)
RUFF_TARGET_VERSION_SERVER=$(awk '/(target-version).*=.*(.*)$/ { print $1 "=" $3 }' server/pyproject.toml)
RUFF_TARGET_VERSION_WEB=$(awk '/(target-version).*=.*(.*)$/ { print $1 "=" $3 }' web/pyproject.toml)
RUFF_OUTPUT_CLIENT=$(ruff check ${RUFF_TARGET_VERSION_CLIENT:+ --config "$RUFF_TARGET_VERSION_CLIENT"} --config ruff-strict.toml --extend-ignore=I001 --exit-zero --output-format concise client)
RUFF_OUTPUT_COMMON=$(ruff check ${RUFF_TARGET_VERSION_COMMON:+ --config "$RUFF_TARGET_VERSION_COMMON"} --config ruff-strict.toml --extend-ignore=I001 --exit-zero --output-format concise common)
RUFF_OUTPUT_DEBUGGING=$(ruff check ${RUFF_TARGET_VERSION_DEBUGGING:+ --config "$RUFF_TARGET_VERSION_DEBUGGING"} --config ruff-strict.toml --extend-ignore=I001 --exit-zero --output-format concise debugging)
RUFF_OUTPUT_POSTGRESQL=$(ruff check ${RUFF_TARGET_VERSION_POSTGRESQL:+ --config "$RUFF_TARGET_VERSION_POSTGRESQL"} --config ruff-strict.toml --extend-ignore=I001 --exit-zero --output-format concise postgresql)
...
FLAKE8_OUTPUT_CLIENT=$(flake8 --ignore E203,E501,E741,W503 client/ --exit-zero)
FLAKE8_OUTPUT_COMMON=$(flake8 --ignore E203,E501,E741,W503 common/ --exit-zero)
FLAKE8_OUTPUT_DEBUGGING=$(flake8 --ignore E203,E501,E741,W503 debugging/ --exit-zero)
FLAKE8_OUTPUT_POSTGRESQL=$(flake8 --ignore E203,E501,E741,W503 postgresql/ --exit-zero)
FLAKE8_OUTPUT_SERVER=$(flake8 --ignore E203,E501,E741,W503 server/ --exit-zero)
FLAKE8_OUTPUT_WEB=$(flake8 --ignore E203,E501,E741,W503 web/ --exit-zero)
